...
class NewFeaturesTest {
    ...
    class UnitsAliasesTest {
        ...
        @Test
        @DisplayName("formatEther(1.5e18 wei) == \"1.5\"")
    ...
    @Nested
    @DisplayName("Chain — new networks")
    class ChainNewNetworksTest {

        @Test
        @DisplayName("CELO has chainId 42220")
        void celo_id() {
            assertEquals(42220L, Chain.CELO.id());
        }

        @Test
        @DisplayName("MANTLE has chainId 5000")
        void mantle_id() {
            assertEquals(5000L, Chain.MANTLE.id());
        }

        @Test
        @DisplayName("MODE has chainId 34443")
        void mode_id() {
            assertEquals(34443L, Chain.MODE.id());
        }

        @Test
        @DisplayName("TAIKO has chainId 167000")
        void taiko_id() {
            assertEquals(167000L, Chain.TAIKO.id());
        }

        @Test
        @DisplayName("POLYGON_ZKEVM has chainId 1101")
        void polygon_zkevm_id() {
            assertEquals(1101L, Chain.POLYGON_ZKEVM.id());
        }

        @Test
        @DisplayName("ZKSYNC_SEPOLIA has chainId 300")
        void zksync_sepolia_id() {
            assertEquals(300L, Chain.ZKSYNC_SEPOLIA.id());
        }

        @Test
        @DisplayName("ARBITRUM_NOVA has chainId 42170")
        void arbitrum_nova_id() {
            assertEquals(42170L, Chain.ARBITRUM_NOVA.id());
        }

        @Test
        @DisplayName("All new chains have valid HTTPS public RPC URLs")
        void new_chains_have_rpcs() {
            for (Chain c :
                ...
                assertTrue(
                        c.publicRpc().startsWith("https://"),
                        c.name() + " RPC must be HTTPS: " + c.publicRpc());
            }
        }

        @Test
        @DisplayName("All new chains have explorer URLs")
        ...
        @Test
        @DisplayName("New chains are found by fromId()")
        ...
        @Test
        @DisplayName("New L2 chains are identified as L2")
        ...
        @Test
        @DisplayName("ZKSYNC_SEPOLIA is a testnet")
        void zksync_sepolia_is_testnet() {
            assertTrue(Chain.ZKSYNC_SEPOLIA.isTestnet());
        }

        @Test
        @DisplayName("ARBITRUM_NOVA is NOT a testnet")
        void arbitrum_nova_not_testnet() {
            assertFalse(Chain.ARBITRUM_NOVA.isTestnet());
        }

        @Test
        @DisplayName("Total Chain count is at least 25 (was 18)")
        void chain_count() {
            assertTrue(
                    Chain.values().length >= 25,
    ...
    @Nested
    @DisplayName("Multicall3 — tryExecute / TryResult")
    class MulticallTryTest {
        ...
        static byte[] buildAggregate3Response(byte[][] returnDatas, boolean[] successes) {
            ...
            try {
                ...
                for (int i = 0; i < n; i++) {
                    byte[] success = new byte[32];
                    success[31] = (byte) (successes[i] ? 1 : 0);
                    bb.write(success);
                    int dataLen = returnDatas[i].length;
                    int pad = ((dataLen + 31) / 32) * 32;
                    byte[] bytesLen = new byte[32];
                    bytesLen[31] = (byte) dataLen;
                    bb.write(bytesLen);
        ...
        @Test
        @DisplayName("tryExecute() returns empty list for empty Multicall")
        ...
        @Test
        @DisplayName("TryResult.success=true has non-null value")
        void try_result_success() throws Exception {
            try (var rpc = new RpcMock()) {
                byte[] encodedResponse =
                        buildAggregate3Response(
        ...
        @Test
        @DisplayName("TryResult.success=false has revertReason (may be empty)")
        void try_result_failure() throws Exception {
            try (var rpc = new RpcMock()) {
                byte[] encodedResponse =
                        buildAggregate3Response(new byte[][] {new byte[0]}, new boolean[] {false});
                rpc.enqueue("\"0x" + io.jeth.util.Hex.encode(encodedResponse).substring(2) + "\"");
        ...
        @Test
        @DisplayName("TryResult.as() throws on failed result")
        ...
        @Test
        @DisplayName("TryResult.opt() returns empty Optional on failure")
        ...
        @Test
        @DisplayName("TryResult.opt() returns value on success")
        ...
        @Test
        @DisplayName("TryResult.toString() shows success/fail")
        void try_result_to_string() {
            ...
            assertTrue(
                    fail.toString().contains("fail") || fail.toString().contains("revert reason"));
        }

        @Test
        @DisplayName("TryResult.isNull() is true when value == null")
    ...
    @Nested
    @DisplayName("Wallet.connect() → ConnectedWallet")
    class ConnectedWalletTest {
        ...
        @Test
        @DisplayName("connect() returns non-null ConnectedWallet")
        ...
        @Test
        @DisplayName("ConnectedWallet.getAddress() == wallet.getAddress()")
        ...
        @Test
        @DisplayName("ConnectedWallet.getWallet() returns the original wallet")
        ...
        @Test
        @DisplayName("ConnectedWallet.getClient() returns the EthClient")
        ...
        @Test
        @DisplayName("sendEth() makes multiple RPC calls (chainId, nonce, estimateGas, block, tip)")
        void send_eth_makes_rpc_calls() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                rpc.enqueueJson(
                        "{\"number\":\"0x12\",\"baseFeePerGas\":\"0x3B9ACA00\","
        ...
        @Test
        @DisplayName("ConnectedWallet.toString() is human-readable")
        void connected_to_string() throws Exception {
            try (var rpc = new RpcMock()) {
                var connected = WALLET.connect(rpc.client());
                String s = connected.toString();
                assertTrue(
                        s.contains("0x") || s.contains("Wallet") || s.contains("Connected"),
    ...
    @Nested
    @DisplayName("Contract.queryFilter()")
    class QueryFilterTest {
        ...
        @Test
        @DisplayName("queryFilter() passes correct fromBlock and toBlock to getLogs")
        void query_filter_block_range() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                assertTrue(
                        body.contains("0x" + Long.toHexString(18_000_000L)),
                        "fromBlock must be in request: " + body);
            }
        }

        @Test
        @DisplayName("queryFilter() with 1-arg (fromBlock only) uses 'latest' as toBlock")
        ...
        @Test
        @DisplayName("queryFilter() decodes matching logs into DecodedEvents")
        ...
        @Test
        @DisplayName("queryFilter() auto-chunks when range > MAX_LOG_CHUNK")
