...
class AbiDecodeErrorTest {
    ...
    @Test
    @DisplayName("decode null → generic revert message")
    void decode_null() {
        assertFalse(AbiDecodeError.decode((String) null).isEmpty());
    }

    @Test
    @DisplayName("decode empty → generic revert message")
    void decode_empty() {
        assertFalse(AbiDecodeError.decode("").isEmpty());
    }

    @Test
    @DisplayName("decode 0x → generic revert message")
    void decode_0x() {
        assertFalse(AbiDecodeError.decode("0x").isEmpty());
    }

    @Test
    @DisplayName("decode Error(string) — standard revert")
    void decode_error_string() {
        byte[] msg =
                AbiCodec.encode(
                        new AbiType[] {AbiType.STRING},
                        new Object[] {"ERC20: transfer amount exceeds balance"});
    ...
    @Test
    @DisplayName("decode Error(string) — short message")
    ...
    @Test
    @DisplayName("decode Panic(0x01) — assertion failed")
    void decode_panic_assert() {
        byte[] data =
                AbiCodec.encode(new AbiType[] {AbiType.UINT256}, new Object[] {BigInteger.ONE});
    ...
    @Test
    @DisplayName("decode Panic(0x11) — arithmetic overflow")
    void decode_panic_overflow() {
        byte[] data =
                AbiCodec.encode(
                        new AbiType[] {AbiType.UINT256}, new Object[] {BigInteger.valueOf(0x11)});
        assertTrue(
                AbiDecodeError.decode(concat(Hex.decode("4e487b71"), data)).contains("overflow"));
    }

    @Test
    @DisplayName("decode Panic(0x12) — division by zero")
    void decode_panic_divzero() {
        byte[] data =
                AbiCodec.encode(
                        new AbiType[] {AbiType.UINT256}, new Object[] {BigInteger.valueOf(0x12)});
        String r = AbiDecodeError.decode(concat(Hex.decode("4e487b71"), data));
        assertTrue(r.contains("zero") || r.contains("division"), "Got: " + r);
    }

    @Test
    @DisplayName("decode Panic(0x32) — array out of bounds")
    void decode_panic_bounds() {
        byte[] data =
                AbiCodec.encode(
                        new AbiType[] {AbiType.UINT256}, new Object[] {BigInteger.valueOf(0x32)});
        assertTrue(AbiDecodeError.decode(concat(Hex.decode("4e487b71"), data)).contains("bounds"));
    }

    @Test
    @DisplayName("decode custom error — shows selector as 0xXXXXXXXX")
    void decode_custom() {
    ...
    @Test
    @DisplayName("errorSelector computes 4-byte hex selector")
    ...
    @Test
    @DisplayName("decode from hex string (RPC error format)")
