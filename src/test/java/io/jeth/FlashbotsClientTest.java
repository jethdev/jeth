...
/**
 * FlashbotsClient tests — covers:
 * - sendBundle: HTTP method, URL, X-Flashbots-Signature header, response parsing
 * - sendPrivateTransaction: body params, response
 * - cancelPrivateTransaction
 * - simulate: parses SimResult
 * - sign(): EIP-191 signing
 * - FlashbotsBundle builder
 * - relay URL constants
 */
class FlashbotsClientTest {
    ...
    @Test @DisplayName("RELAY_MAINNET is a valid HTTPS URL")
    void relay_mainnet_https() {
        assertTrue(FlashbotsClient.RELAY_MAINNET.startsWith("https://"));
    }

    @Test @DisplayName("MEV_BLOCKER is a valid HTTPS URL")
    void mev_blocker_https() {
        assertTrue(FlashbotsClient.MEV_BLOCKER.startsWith("https://"));
    }

    @Test @DisplayName("All relay URLs start with https://")
    void all_relays_https() {
        for (String url : List.of(FlashbotsClient.RELAY_MAINNET, FlashbotsClient.RELAY_GOERLI,
                FlashbotsClient.MEV_BLOCKER, FlashbotsClient.TITAN_RELAY, FlashbotsClient.BEAVER_RELAY)) {
    ...
    @Test @DisplayName("sign() returns non-empty hex string")
    void sign_non_empty() {
    ...
    @Test @DisplayName("sign() is deterministic for same input")
    ...
    @Test @DisplayName("sign() produces different signatures for different bodies")
    void sign_different_bodies() {
        var client = FlashbotsClient.of("https://relay.flashbots.net", WALLET);
        assertNotEquals(client.sign("{\"body\":1}"), client.sign("{\"body\":2}"));
    }

    @Test @DisplayName("sign() output is 0x-prefixed hex (65 bytes = 130 hex chars + 0x)")
    void sign_format() {
        var client = FlashbotsClient.of("https://relay.flashbots.net", WALLET);
        String sig = client.sign("some body content");
        assertTrue(sig.startsWith("0x") || sig.length() == 130,
    ...
    @Test @DisplayName("sendBundle() sends POST to relay URL")
    ...
    @Test @DisplayName("sendBundle() sets X-Flashbots-Signature header")
    void send_bundle_auth_header() throws Exception {
        try (var server = mockRelay("{\"id\":1,\"result\":{\"bundleHash\":\"0xabc\"}}")) {
            ...
            assertTrue(header.contains(WALLET.getAddress()),
                "Header must include signer address: " + header);
            assertTrue(header.contains(":"), "Header format: address:signature");
        }
    }

    @Test @DisplayName("sendBundle() includes blockNumber in request body")
    void send_bundle_block_number_in_body() throws Exception {
        try (var server = mockRelay("{\"id\":1,\"result\":{\"bundleHash\":\"0xabc\"}}")) {
            ...
            assertTrue(body.contains("0x11a2a40") || body.contains("11a2a40"),
                "Must include block number 18500000 hex: " + body);
        }
    }

    @Test @DisplayName("sendBundle() with range sets maxBlock in body")
    void send_bundle_max_block() throws Exception {
        try (var server = mockRelay("{\"id\":1,\"result\":{\"bundleHash\":\"0xabc\"}}")) {
            var client = FlashbotsClient.of(server.url("/").toString(), WALLET);
            client.sendBundle(FlashbotsBundle.of().tx("0xdeadbeef").build(),
                    18_000_000L, 18_000_003L).join();
            String body = server.takeRequest().getBody().readUtf8();
            assertTrue(body.contains("maxBlock"), body);
        }
    }

    @Test @DisplayName("sendBundle() throws FlashbotsException on relay error response")
    void send_bundle_relay_error() throws Exception {
        try (var server = mockRelay("{\"id\":1,\"error\":{\"message\":\"bundle not valid\"}}")) {
            var client = FlashbotsClient.of(server.url("/").toString(), WALLET);
            assertThrows(Exception.class,
                () -> client.sendBundle(FlashbotsBundle.of().tx("0xdeadbeef").build(), 1L).join());
        }
    }

    // ─── sendPrivateTransaction ───────────────────────────────────────────────

    @Test @DisplayName("sendPrivateTransaction() sends eth_sendPrivateTransaction")
    ...
    @Test @DisplayName("sendPrivateTransaction(tx, maxBlock, fast) includes preferences")
    ...
    @Test @DisplayName("cancelPrivateTransaction() sends eth_cancelPrivateTransaction")
    ...
    @Test @DisplayName("simulate() parses FlashbotsSimResult from relay response")
    void simulate_parses_result() throws Exception {
        String simResponse = "{\"id\":1,\"result\":{"
    ...
    @Test @DisplayName("SimResult.anyReverted() true when a tx has error field")
    ...
    @Test @DisplayName("SimResult.minerPaymentEth() is human-readable")
    void sim_result_miner_payment_eth() {
        var result = new FlashbotsClient.FlashbotsSimResult(
    ...
    @Test @DisplayName("FlashbotsBundle.of().tx().build() creates valid bundle")
    ...
    @Test @DisplayName("FlashbotsBundle with multiple txs preserves order")
    void bundle_preserves_order() {
        var bundle = FlashbotsBundle.of()
                .tx("0xTx1").tx("0xTx2").tx("0xTx3").build();
    ...
    @Test @DisplayName("FlashbotsBundle.allowRevert() adds tx hash to revertingTxHashes")
    ...
    @Test @DisplayName("FlashbotsBundle.txs(list) adds multiple txs")
    void bundle_txs_list() {
        var bundle = FlashbotsBundle.of()
                .txs(List.of("0xA", "0xB", "0xC")).build();
        assertEquals(3, bundle.size());
    }

    @Test @DisplayName("Empty bundle throws IllegalStateException")
    void bundle_empty_throws() {
        assertThrows(IllegalStateException.class,
                () -> FlashbotsBundle.of().build());
    }

    @Test @DisplayName("FlashbotsBundle.rawTxs() is unmodifiable")
    void bundle_immutable() {
        var bundle = FlashbotsBundle.of().tx("0xTx").build();
        assertThrows(UnsupportedOperationException.class,
                () -> bundle.rawTxs().add("0xHacked"));
    }

    @Test @DisplayName("FlashbotsBundle.toString() is human-readable")
    ...
    @Test @DisplayName("mainnet(wallet) creates client pointing to mainnet relay")
    ...
    @Test @DisplayName("of(url, wallet, timeout) accepts custom timeout")
    void custom_timeout_factory() {
        assertDoesNotThrow(() ->
                FlashbotsClient.of("https://relay.flashbots.net", WALLET, Duration.ofSeconds(30)));
    }
