...
class EthClientTest {
    ...
    @Test @DisplayName("getChainId parses hex response")
    ...
    @Test @DisplayName("getBlockNumber parses hex response")
    ...
    @Test @DisplayName("getNetworkId returns string")
    ...
    @Test @DisplayName("getBalance returns wei amount")
    void balance() throws Exception {
        try (var rpc = new RpcMock()) {
            BigInteger oneEth = new BigInteger("1000000000000000000");
            rpc.enqueueHex(oneEth);
            assertEquals(oneEth, rpc.client().getBalance("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266").join());
        }
    }

    @Test @DisplayName("getBalance with block tag sends both params")
    ...
    @Test @DisplayName("getTransactionCount (nonce)")
    void nonce() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueHex(7L);
            assertEquals(7L, rpc.client().getTransactionCount("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266").join());
        }
    }

    @Test @DisplayName("getCode returns hex bytecode")
    void getCode() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueStr("6080604052");
            assertEquals("6080604052", rpc.client().getCode("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48").join());
        }
    }

    @Test @DisplayName("isContract true when code non-empty")
    ...
    @Test @DisplayName("isContract false for EOA (empty bytecode)")
    ...
    @Test @DisplayName("getGasPrice parses hex")
    ...
    @Test @DisplayName("getBlock parses all standard fields")
    ...
    @Test @DisplayName("getBlock returns null for unknown block")
    ...
    @Test @DisplayName("getBlockByNumber sends correct number param")
    void getBlockByNumber() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"hash\":\"0xabc\",\"number\":\"0x5\",\"timestamp\":\"0x0\",\"gasLimit\":\"0x0\",\"gasUsed\":\"0x0\",\"transactions\":[]}");
    ...
    @Test @DisplayName("getTransaction parses all EIP-1559 fields")
    ...
    @Test @DisplayName("getTransactionReceipt parses status and gasCostWei")
    ...
    @Test @DisplayName("getTransactionReceipt status 0 = failed")
    void getReceiptFailed() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"transactionHash\":\"0xtx\",\"status\":\"0x0\",\"blockNumber\":\"0x1\",\"gasUsed\":\"0x5208\",\"logs\":[]}");
            assertFalse(rpc.client().getTransactionReceipt("0xtx").join().isSuccess());
        }
    }

    @Test @DisplayName("getTransactionReceipt null = pending/unknown")
    ...
    @Test @DisplayName("sendRawTransaction returns hash")
    ...
    @Test @DisplayName("call returns raw hex result")
    void ethCall() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            var req = EthModels.CallRequest.builder()
                    .to("0xToken").data("0x70a08231" + "0".repeat(64)).build();
            String result = rpc.client().call(req).join();
            assertEquals(new BigInteger("1000000"), Hex.toBigInteger(result));
        }
    }

    @Test @DisplayName("estimateGas returns gas as BigInteger")
    ...
    @Test @DisplayName("getLogs parses log array and matchesTopic0")
    void getLogs() throws Exception {
        String topic0 = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef";
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("[{\"address\":\"0xToken\",\"topics\":[\"" + topic0 + "\",\"0x0\",\"0x0\"],"
    ...
    @Test @DisplayName("getLogs empty array")
    ...
    @Test @DisplayName("suggestEip1559Fees returns [maxFee, tip] both positive")
    void suggestFees() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"hash\":\"0xabc\",\"number\":\"0x1\",\"timestamp\":\"0x0\","
    ...
    @Test @DisplayName("getFeeHistory parses reward array")
    void feeHistory() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"oldestBlock\":\"0x1\",\"baseFeePerGas\":[\"0x4a817c800\"],"
