...
class ContractTest {
    ...
    @Test
    @DisplayName("fn().returns().call().as(BigInteger) decodes uint256")
    ...
    @Test
    @DisplayName("fn().call() encodes correct function selector in request")
    ...
    @Test
    @DisplayName("fn().call() targeting correct contract address")
    void call_target_address() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            String body = rpc.takeRequest().getBody().readUtf8();
            assertTrue(
                    body.toLowerCase().contains(ADDR.toLowerCase().substring(2)),
                    "Contract address must appear in eth_call params");
        }
    }

    @Test
    @DisplayName("fn().returns(string).call() decodes string")
    ...
    @Test
    @DisplayName("fn().returns(bool).call() decodes boolean true")
    ...
    void send_fires_rpc() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            var contract = new Contract(ADDR, rpc.client());
            String txHash =
                    contract.fn("transfer(address,uint256)")
                            .send(WALLET, USER, BigInteger.valueOf(1_000_000L))
                            .join();
