...
/**
 * Extended Multicall3 tests covering executeWithResults, tryExecute, getEthBalances,
 * getTokenBalances, and FluentBuilder.executeAs.
 */
class Multicall3ExtendedTest {
    ...
    @Test
    @DisplayName("executeWithResults() returns empty list when no calls added")
    ...
    @Test
    @DisplayName("executeWithResults() returns Result with success=true and decoded value")
    ...
    @Test
    @DisplayName("executeWithResults() Result: index matches insertion order")
    ...
    @Test
    @DisplayName("executeWithResults() failed optional call: success=false, value=null")
    ...
    @Test
    @DisplayName("executeWithResults(blockTag) forwards block parameter")
    ...
    @Test
    @DisplayName("getEthBalances() returns empty list for empty input")
    ...
    @Test
    @DisplayName("getEthBalances() returns BigInteger balances in input order")
    void get_eth_balances_two_addresses() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            List<BigInteger> bals =
                    Multicall3.getEthBalances(rpc.client(), List.of(ADDR_A, ADDR_B)).join();
    ...
    @Test
    @DisplayName("getEthBalances() makes exactly one RPC call for N addresses")
    void get_eth_balances_single_rpc() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            Multicall3.getEthBalances(
                            rpc.client(), List.of(ADDR_A, ADDR_B, Wallet.create().getAddress()))
                    .join();
    ...
    @Test
    @DisplayName("getTokenBalances() returns empty list for empty token list")
    void get_token_balances_empty() throws Exception {
        try (var rpc = new RpcMock()) {
            List<BigInteger> bals =
                    Multicall3.getTokenBalances(rpc.client(), ADDR_A, List.of()).join();
            assertTrue(bals.isEmpty());
            assertEquals(0, rpc.requestCount());
        }
    }
    ...
    @Test
    @DisplayName("getTokenBalances() returns balances for multiple tokens in order")
    void get_token_balances_multiple() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            List<BigInteger> bals =
                    Multicall3.getTokenBalances(rpc.client(), ADDR_A, tokens).join();
    ...
    @Test
    @DisplayName("getTokenBalances() uses 1 RPC call for N tokens")
                    ...
                    encodeMulticallResponse(
                            List.of(
                                    new CallResult(
                                            ...
                                            AbiCodec.encode(
                                                    new AbiType[] {AbiType.UINT256},
                                                    new Object[] {BigInteger.valueOf(100)})),
                                    new CallResult(
                                            true,
                                            AbiCodec.encode(
                                                    new AbiType[] {AbiType.UINT256},
                                                    new Object[] {BigInteger.valueOf(200)})))));

            Multicall3.getTokenBalances(rpc.client(), ADDR_A, List.of("0xToken1", "0xToken2"))
                    .join();
    ...
    @Test
    @DisplayName("FluentBuilder.executeAs(BigInteger.class) returns typed list")
    ...
    @Test
    @DisplayName("FluentBuilder.optional() treats failure as null (not exception)")
    void fluent_optional_failure() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueue(encodeMulticallResponse(List.of(new CallResult(false, new byte[0]))));
    ...
    @Test
    @DisplayName("FluentBuilder.executeWithResults() returns Result wrappers")
    ...
    /**
     * Encode the Multicall3 aggregate3 return format: (bool success, bytes returnData)[]
     * ABI-encoded.
     */
    static String encodeMulticallResponse(List<CallResult> calls) {
        ...
        // For the mock, encode as: array length + n * (bool slot + bytes offset + bytes length +
        // bytes data padded)
        // ABI for (bool,bytes)[]:
    ...
    static io.jeth.crypto.Wallet Wallet_create() {
        return io.jeth.crypto.Wallet.create();
    }

    static class Wallet {
        static io.jeth.crypto.Wallet create() {
            return io.jeth.crypto.Wallet.create();
        }
    }
}
