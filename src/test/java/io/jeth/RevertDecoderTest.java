...
/**
 * RevertDecoder tests — covers: - Error(string) and Panic passthrough to AbiDecodeError - OZ custom
 * error resolution - Uniswap custom error resolution - Common error resolution - Custom
 * registration - selector() computation - lookup() optional - Edge cases (null, empty, too-short
 * data)
 */
class RevertDecoderTest {
    ...
    @Test
    @DisplayName("decodes Error(string) → 'execution reverted: ...'")
    ...
    @Test
    @DisplayName("decodes Panic(0x11) → arithmetic overflow")
    void panic_overflow() {
    ...
    @Test
    @DisplayName("resolves OZ ERC-20 InsufficientBalance custom error")
    void oz_erc20_insufficient_balance() {
        ...
        assertTrue(
                result.contains("ERC20InsufficientBalance") || result.contains("OZ"),
                "Expected OZ ERC-20 error, got: " + result);
    }

    @Test
    @DisplayName("resolves OZ ERC-20 InvalidSender custom error")
    void oz_erc20_invalid_sender() {
        String sel = RevertDecoder.selector("ERC20InvalidSender(address)");
        assertFalse(
                RevertDecoder.decode(sel).contains("custom error"),
                "Should resolve to named error, not 'custom error'");
    }

    @Test
    @DisplayName("resolves OZ ERC-721 NonexistentToken custom error")
    ...
    @Test
    @DisplayName("resolves OZ AccessControl UnauthorizedAccount")
    ...
    @Test
    @DisplayName("resolves OZ Ownable UnauthorizedAccount")
    ...
    @Test
    @DisplayName("resolves OZ ReentrancyGuard error")
    void oz_reentrancy_guard() {
        String sel = RevertDecoder.selector("ReentrancyGuardReentrantCall()");
        assertTrue(
                RevertDecoder.decode(sel).contains("ReentrancyGuard")
    ...
    @Test
    @DisplayName("resolves Uniswap V3TooLittleReceived")
    ...
    @Test
    @DisplayName("resolves Uniswap DeadlinePassed")
    ...
    @Test
    @DisplayName("resolves Uniswap ExecutionFailed")
    ...
    @Test
    @DisplayName("resolves common Unauthorized()")
    ...
    @Test
    @DisplayName("resolves common ZeroAddress()")
    ...
    @Test
    @DisplayName("resolves common SlippageExceeded()")
    ...
    @Test
    @DisplayName("registerError() resolves newly added error")
    void register_custom_error() {
        ...
        String result = RevertDecoder.decode(sel);
        assertTrue(
                result.contains("InsufficientCollateralXyz") || result.contains("MyProtocol"),
                "Expected custom error to be resolved, got: " + result);
    }

    @Test
    @DisplayName("register() with explicit selector resolves correctly")
    ...
    @Test
    @DisplayName("registerError() is idempotent (registering twice doesn't throw)")
    ...
    @Test
    @DisplayName("lookup() returns empty for unknown selector")
    void lookup_unknown() {
        assertEquals(Optional.empty(), RevertDecoder.lookup("0x00000000"));
    }

    @Test
    @DisplayName("lookup() is case-insensitive (uppercase hex)")
    void lookup_case_insensitive() {
        ...
        assertTrue(
                RevertDecoder.lookup("0x" + upper).isPresent()
    ...
    @Test
    @DisplayName("registry() contains many entries (OZ + Uniswap + Aave + common)")
    void registry_has_entries() {
        assertTrue(
                RevertDecoder.registry().size() >= 50,
                "Expected at least 50 registered errors, got: " + RevertDecoder.registry().size());
    }

    @Test
    @DisplayName("registry() is unmodifiable")
    ...
    @Test
    @DisplayName("decode(null) returns 'execution reverted' (no throw)")
    void decode_null() {
    ...
    @Test
    @DisplayName("decode('') returns graceful message")
    void decode_empty() {
    ...
    @Test
    @DisplayName("decode('0x') returns graceful message")
    void decode_0x() {
    ...
    @Test
    @DisplayName("decode with unknown 4-byte selector returns 'custom error 0x...'")
    ...
    @Test
    @DisplayName("decode(byte[]) works the same as decode(String)")
