...
/**
 * Tests for BIP-39 mnemonic validation (validate, isValid, MnemonicException).
 * Covers: word count, unknown words, checksum, passphrase, edge cases.
 */
class MnemonicValidationTest {

    static final String VALID_12 = "test test test test test test test test test test test junk";
    static final String VALID_24 = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art";

    // ─── isValid / validate basics ────────────────────────────────────────────

    @Test @DisplayName("isValid() returns true for a known-good 12-word mnemonic")
    void valid_12_word() {
        assertTrue(HdWallet.isValid(VALID_12));
    }

    @Test @DisplayName("isValid() returns true for a known-good 24-word mnemonic")
    void valid_24_word() {
        assertTrue(HdWallet.isValid(VALID_24));
    }

    @Test @DisplayName("validate() does not throw for valid mnemonic")
    void validate_no_throw() {
        assertDoesNotThrow(() -> HdWallet.validate(VALID_12));
    }

    @Test @DisplayName("fromMnemonic() still works — validate is called internally")
    ...
    @Test @DisplayName("11-word mnemonic is rejected")
    void reject_11_words() {
        var ex = assertThrows(HdWallet.MnemonicException.class,
            () -> HdWallet.validate("word word word word word word word word word word word"));
        assertTrue(ex.getMessage().contains("word count") || ex.getMessage().contains("11"),
            "Message should mention word count: " + ex.getMessage());
    }

    @Test @DisplayName("13-word mnemonic is rejected")
    void reject_13_words() {
        assertThrows(HdWallet.MnemonicException.class,
            () -> HdWallet.validate("abandon ".repeat(13).trim()));
    }

    @Test @DisplayName("empty string is rejected")
    void reject_empty() {
        assertThrows(HdWallet.MnemonicException.class, () -> HdWallet.validate(""));
    }

    @Test @DisplayName("null is rejected")
    void reject_null() {
        assertThrows(HdWallet.MnemonicException.class, () -> HdWallet.validate(null));
    }

    @Test @DisplayName("blank/whitespace is rejected")
    void reject_blank() {
        assertThrows(HdWallet.MnemonicException.class, () -> HdWallet.validate("   "));
    }
    ...
    @Test @DisplayName("Mnemonic with a non-BIP39 word is rejected")
    void reject_unknown_word() {
        ...
        assertTrue(ex.getMessage().toLowerCase().contains("badword") ||
                   ex.getMessage().toLowerCase().contains("unknown"),
            "Message should identify the bad word: " + ex.getMessage());
    }

    @Test @DisplayName("Mnemonic with a number token is rejected")
    ...
    @Test @DisplayName("Mnemonic with wrong last word (bad checksum) is rejected")
    ...
    void reject_swapped_words() {
        ...
        String tmp = words[0]; words[0] = words[1]; words[1] = tmp;
        ...
        assertDoesNotThrow(() -> {
            try { HdWallet.validate(swapped); } catch (HdWallet.MnemonicException e) { /* expected */ }
        });
    }

    // ─── isValid vs validate ──────────────────────────────────────────────────

    @Test @DisplayName("isValid() returns false instead of throwing for invalid mnemonic")
    ...
    @Test @DisplayName("isValid() agrees with validate() on valid mnemonic")
    ...
    @Test @DisplayName("Validation is case-insensitive (uppercase input)")
    void case_insensitive() {
        assertTrue(HdWallet.isValid(VALID_12.toUpperCase()));
    }

    @Test @DisplayName("Validation handles extra whitespace")
    ...
    @Test @DisplayName("MnemonicException is a RuntimeException")
    void exception_type() {
        var ex = assertThrows(HdWallet.MnemonicException.class,
    ...
    @Test @DisplayName("MnemonicException message starts with 'Invalid BIP-39 mnemonic'")
    void exception_message_prefix() {
        var ex = assertThrows(HdWallet.MnemonicException.class,
            () -> HdWallet.validate("one two three four five six seven eight nine ten eleven twelve"));
        assertTrue(ex.getMessage().startsWith("Invalid BIP-39 mnemonic"),
