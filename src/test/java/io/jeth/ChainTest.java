...
class ChainTest {
    ...
    @Test @DisplayName("MAINNET chainId = 1")
    void mainnet_id() { assertEquals(1L, Chain.MAINNET.id()); }

    @Test @DisplayName("All chains have valid public HTTPS RPC")
    ...
    @Test @DisplayName("All chains have an explorer URL")
    void all_explorers() {
        for (Chain c : Chain.values())
            assertNotNull(c.explorer(), c.name() + " has null explorer");
    }

    @Test @DisplayName("fromId() finds major chains")
    void from_id() {
    ...
    @Test @DisplayName("fromId() returns empty for unknown chain")
    void from_id_unknown() { assertTrue(Chain.fromId(999999L).isEmpty()); }

    @Test @DisplayName("requireId() throws for unknown chain")
    void require_id_throws() {
        assertThrows(IllegalArgumentException.class, () -> Chain.requireId(999999L));
    }

    @Test @DisplayName("isL2() correct for L1 vs L2")
    void is_l2() {
    ...
    @Test @DisplayName("isTestnet() identifies testnets correctly")
    void is_testnet() {
    ...
    @Test @DisplayName("txUrl() builds correct etherscan link")
    void tx_url() {
        assertEquals("https://etherscan.io/tx/0xabc", Chain.MAINNET.txUrl("0xabc"));
    }

    @Test @DisplayName("addressUrl() builds correct basescan link")
    void address_url() {
        assertEquals("https://basescan.org/address/0xAddr", Chain.BASE.addressUrl("0xAddr"));
    }

    @Test @DisplayName("rpc(key) interpolates API key")
    void rpc_with_key() {
        String url = Chain.MAINNET.rpc("mykey");
        assertTrue(url.contains("mykey") || url.equals(Chain.MAINNET.publicRpc()),
