...
/**
 * Coverage tests for the remaining gap areas identified in the systematic audit: MiddlewareProvider
 * (retry, logging, avgLatencyMs), ERC721 (ownerOf/tokenURI/getApproved), AaveV3
 * (getUserAccountData/healthFactor/isLiquidatable), GnosisSafe (packSorted/getOwners),
 * BundlerClient (getUserOperationByHash), ERC20 (balanceOfFormatted/permit), PriceOracle
 * (spot/twap/roundData), and utility helpers (Address.isZero, Units.toEther, Hex.isHex/zeroPad,
 * Keccak.hashHex, GasEstimator.estimateGasCost/priorityFeeGwei).
 */
class MiscCoverageTest {
    ...
    @Nested
    @DisplayName("MiddlewareProvider retry + logging")
    class MiddlewareProviderExtended {

        @Test
        @DisplayName("withRetry: retries on transient error, succeeds on second attempt")
            ...
            try (var rpc = new RpcMock()) {
                ...
                rpc.enqueue(
                        "{\"jsonrpc\":\"2.0\",\"id\":1,\"error\":{\"code\":-32000,\"message\":\"timeout\"}}"
                                .substring(0)); // can't enqueue error directly, use null trick
                // Actually enqueue a valid response for the retry
                rpc.enqueueHex(42L);

                // Build middleware with retry(2, 1ms delay)
                MiddlewareProvider mp =
                        MiddlewareProvider.wrap(rpc.client().getProvider())
                ...
                var result =
                        mp.send(
                                        new io.jeth.model.RpcModels.RpcRequest(
                                                "eth_blockNumber", java.util.List.of()))
                                .join();
        ...
        @Test
        @DisplayName("withRetry: metrics.retries starts at 0")
        ...
        @Test
        @DisplayName("withLogging: provider works correctly with logging enabled")
        void logging_enabled_works() throws Exception {
            try (var rpc = new RpcMock()) {
                rpc.enqueueHex(100L);
                MiddlewareProvider mp =
                        MiddlewareProvider.wrap(rpc.client().getProvider()).withLogging().build();
                var result =
                        mp.send(
                                        new io.jeth.model.RpcModels.RpcRequest(
                                                "eth_blockNumber", java.util.List.of()))
        ...
        @Test
        @DisplayName("avgLatencyMs: returns 0 before any calls")
        ...
        @Test
        @DisplayName("avgLatencyMs: returns positive value after calls")
        void avg_latency_positive_after_calls() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                mp.send(
                        ...
                        .join();
                assertTrue(
                        mp.getMetrics().avgLatencyMs() >= 0.0,
    ...
    class ERC721Tests {
        ...
        @Test
        @DisplayName("ownerOf() returns address string")
        ...
        @Test
        @DisplayName("ownerOf() sends balanceOf-style eth_call RPC")
        ...
        @Test
        @DisplayName("tokenURI() returns metadata URI string")
        ...
        @Test
        @DisplayName("getApproved() returns approved operator address")
        ...
        @Test
        @DisplayName("getApproved() returns zero address when no approval")
        void get_approved_none() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                assertTrue(
                        result.replace("0x", "").chars().allMatch(c -> c == '0'),
    ...
    class AaveV3Tests {
        ...
        @Test
        @DisplayName("getUserAccountData returns AccountData with all fields")
        ...
        @Test
        @DisplayName("AccountData.healthFactorEther() converts from 18-decimal fixed point")
        ...
        @Test
        @DisplayName("AccountData.isLiquidatable() true when HF < 1.0")
        void is_liquidatable_true() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                AaveV3.AccountData data =
                        new AaveV3(POOL, rpc.client()).getUserAccountData("0xUser").join();
                assertTrue(data.isLiquidatable(), "HF < 1.0 must be liquidatable");
            }
        }

        @Test
        @DisplayName("AccountData.isLiquidatable() false when HF > 1.0")
    ...
    class GnosisSafeTests {
        ...
        @Test
        @DisplayName("packSorted: single signature is 65 bytes")
        ...
        @Test
        @DisplayName("packSorted: two signatures = 130 bytes, sorted by address")
        ...
        @Test
        @DisplayName("packSorted: lower address always appears first in output")
        void pack_sorted_order() {
            ...
            Map<String, Signature> sigs2 =
                    Map.of(
                            a2, w2.sign(digest),
                            a1, w1.sign(digest));
            byte[] packed2 = GnosisSafe.packSorted(sigs2);
            assertArrayEquals(
                    packed, packed2, "packSorted must be deterministic regardless of map ordering");
        }

        @Test
        @DisplayName("getOwners sends eth_call to getOwners() function")
    ...
    @Nested
    @DisplayName("BundlerClient extended")
    class BundlerClientExtended {

        @Test
        @DisplayName("getUserOperationByHash sends eth_getUserOperationByHash")
        ...
        @Test
        @DisplayName("getUserOperationByHash returns null when not found")
        ...
        @Test
        @DisplayName("getUserOperationByHash returns JsonNode when found")
        void get_user_op_by_hash_found() throws Exception {
            try (var rpc = new RpcMock()) {
                rpc.enqueue(
                        "{\"sender\":\"0xSender\",\"nonce\":\"0x5\",\"callGasLimit\":\"0x5208\"}");
                BundlerClient bc = BundlerClient.of(rpc.url());
    ...
    class ERC20Extended {
        ...
        @Test
        @DisplayName("balanceOfFormatted returns BigDecimal with correct decimals")
        ...
        @Test
        @DisplayName("balanceOfFormatted: 1 USDC (1_000_000 units, 6 decimals)")
        ...
        @Test
        @DisplayName("getContract returns the underlying Contract instance")
    ...
    @Nested
    @DisplayName("PriceOracle extended")
    class PriceOracleExtended {
        ...
        @Test
        @DisplayName("roundData returns ChainlinkRound with answer and timestamp")
        ...
        @Test
        @DisplayName("roundData: ChainlinkRound.price() applies decimal scaling")
        void round_data_price_scaled() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                assertEquals(
                        0,
                        new BigDecimal("3412.58").compareTo(scaled),
                        "price() must scale correctly: 341258000000 / 10^8 = 3412.58");
            }
        }

        @Test
        @DisplayName("spot returns USD price from Uniswap V3 TWAP (1-second window)")
        void spot_price() throws Exception {
            try (var rpc = new RpcMock()) {
                // spot() uses Uniswap V3 observe() — encode a realistic tick response
                // observe returns (int56[] tickCumulatives, uint160[]
        ...
        @Test
        @DisplayName("twap returns price averaged over window")
    ...
    @Nested
    @DisplayName("GasEstimator extended")
    class GasEstimatorExtended {

        @Test
        @DisplayName("estimateGasCost returns wei cost (gas * gasPrice)")
        ...
        @Test
        @DisplayName("FeeEstimate.priorityFeeGwei() converts to gwei double")
    ...
    class UtilityHelpers {
        ...
        @Test
        @DisplayName("Units.toEther: 0.5 ETH in wei → BigDecimal 0.5")
        void to_ether_half() {
        ...
        @Test
        @DisplayName("Units.toEther(0) = 0")
        void to_ether_zero() {
            assertEquals(BigDecimal.ZERO, Units.toEther(BigInteger.ZERO).stripTrailingZeros());
        }

        @Test
        @DisplayName("Hex.isHex: valid 0x-prefixed hex = true")
        ...
        @Test
        @DisplayName("Hex.isHex: invalid = false")
        ...
        @Test
        @DisplayName("Hex.zeroPad pads hex to 32 bytes")
        void hex_zero_pad() {
        ...
        @Test
        @DisplayName("Hex.zeroPad: already 32 bytes passes through")
        ...
        @Test
        @DisplayName("Keccak.hashHex returns 0x-prefixed 66-char string")
        ...
        @Test
        @DisplayName("Keccak.hashHex: Transfer signature matches known hash")
        void keccak_hash_hex_transfer() {
            String result = Keccak.hashHex("Transfer(address,address,uint256)");
            assertEquals(
                    "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef", result);
        }

        @Test
        @DisplayName("Keccak.hashHex: consistent with hash(byte[])")
