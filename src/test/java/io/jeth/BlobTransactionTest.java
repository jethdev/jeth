...
class BlobTransactionTest {
    ...
    @Test @DisplayName("padBlob of exact blob size returns same size")
    void pad_blob_exact() {
    ...
    @Test @DisplayName("Blob.versioned hash is 32 bytes starting with 0x01")
    ...
    @Test @DisplayName("sign() returns type-3 tx starting with 0x03")
    void sign_type3() {
        ...
        assertNotNull(raw);
        assertTrue(raw.startsWith("0x03"), "Type-3 tx must start with 0x03, got: " + raw.substring(0, Math.min(6, raw.length())));
    }

    @Test @DisplayName("getBlobVersionedHashes returns one hash per blob")
    void blob_versioned_hashes_count() {
        BlobTransaction tx = BlobTransaction.builder()
            .chainId(1L).nonce(0L)
            .to("0xTarget").value(BigInteger.ZERO).gas(BigInteger.valueOf(21000))
