 ...
 *
 * <p>Uses N=4096 (encryptLight) so tests run fast. The production encrypt() uses N=262144 which
 * would be slow in unit tests.
 */
class KeystoreTest {

    // Known test vector: known private key → known address
    private static final String KNOWN_PRIVKEY =
    ...
    void wrongPasswordThrowsKeystoreException() {
        ...
        assertThrows(Keystore.KeystoreException.class, () -> Keystore.decrypt(json, "wrong"));
    }
    ...
    void addressStoredLowerCaseNoPrefix() {
        ...
        assertTrue(
                json.contains(expectedAddr),
                "Keystore should contain lowercase address without 0x");
    }
    ...
    void filenameHasCorrectFormat() {
        ...
        assertTrue(
                fname.endsWith("f39fd6e51aad88f6f4ce6ab8827279cfffb92266"),
                "Filename must end with lowercase address without 0x");
    }

    /**
     * Interoperability test: decrypt a keystore produced by ethers.js. Generated with:
     * ethers.Wallet.fromMnemonic(...).encrypt("testpassword", {scrypt:{N:4096}}) Address:
