...
class UserOperationTest {
    ...
    @Test @DisplayName("getUserOpHash returns 0x-prefixed 32-byte hash")
    void hash_format() {
    ...
    @Test @DisplayName("getUserOpHash is deterministic")
    void hash_deterministic() {
        UserOperation op = minimalOp();
        assertEquals(op.getUserOpHash(ENTRY_POINT, CHAIN_ID),
                     op.getUserOpHash(ENTRY_POINT, CHAIN_ID));
    }

    @Test @DisplayName("getUserOpHash changes with different sender")
    void hash_changes_with_sender() {
        UserOperation op1 = minimalOp();
        UserOperation op2 = UserOperation.builder()
                ...
                .maxPriorityFeePerGas(BigInteger.valueOf(1_000_000_000L))
                .build();
        assertNotEquals(op1.getUserOpHash(ENTRY_POINT, CHAIN_ID),
                        op2.getUserOpHash(ENTRY_POINT, CHAIN_ID));
    }

    @Test @DisplayName("getUserOpHash changes with different chainId")
    ...
    @Test @DisplayName("getUserOpHash changes with different entryPoint")
    ...
    @Test @DisplayName("sign() populates signature field")
    ...
    @Test @DisplayName("sign() v byte is 27 or 28 (Ethereum convention)")
    ...
    @Test @DisplayName("sign() is deterministic for same key and op")
    ...
    @Test @DisplayName("sign() original op unchanged (immutable return)")
    ...
    @Test @DisplayName("toMap() contains all required ERC-4337 fields")
    ...
    @Test @DisplayName("toMap() nonce is 0x-prefixed hex")
    ...
    @Test @DisplayName("toMap() empty initCode defaults to 0x")
    ...
    @Test @DisplayName("getUserOpHash encodes initCode, callData, paymasterAndData as keccak256")
    void hash_uses_keccak_for_dynamic_fields() {
        // Two ops identical except initCode is empty vs "0x" — must hash identically
        UserOperation op1 = UserOperation.builder()
                .sender("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266")
                .nonce(BigInteger.ZERO).callData("0x").callGasLimit(BigInteger.valueOf(100_000))
                .verificationGasLimit(BigInteger.valueOf(150_000)).preVerificationGas(BigInteger.valueOf(21_000))
                .maxFeePerGas(BigInteger.valueOf(30_000_000_000L)).maxPriorityFeePerGas(BigInteger.valueOf(1_000_000_000L))
                .initCode(null).build();
        UserOperation op2 = UserOperation.builder()
                .sender("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266")
                .nonce(BigInteger.ZERO).callData("0x").callGasLimit(BigInteger.valueOf(100_000))
                .verificationGasLimit(BigInteger.valueOf(150_000)).preVerificationGas(BigInteger.valueOf(21_000))
                .maxFeePerGas(BigInteger.valueOf(30_000_000_000L)).maxPriorityFeePerGas(BigInteger.valueOf(1_000_000_000L))
                .initCode("0x").build(); // explicit "0x" same as null/empty
        // Both "null" and "0x" represent empty bytes and should hash identically
        assertEquals(op1.getUserOpHash(ENTRY_POINT, CHAIN_ID),
                     op2.getUserOpHash(ENTRY_POINT, CHAIN_ID),
