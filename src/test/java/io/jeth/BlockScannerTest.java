...
/**
 * BlockScanner tests — covers scan, collect, count, scanMulti, progress, cursor/resume, early exit,
...
class BlockScannerTest {
    ...
    static String transferLog(String from, String to, long value, long blockNumber) {
        ...
        return "{\"address\":\""
                ...
                + "\","
                + "\"transactionHash\":\"0x"
                + String.format("%064x", blockNumber)
                + "\","
                + "\"transactionIndex\":\"0x0\","
                + "\"blockHash\":\"0x"
                + String.format("%064x", blockNumber + 1)
                + "\","
                + "\"logIndex\":\"0x0\","
                + "\"removed\":false}";
    }
    ...
    @Test
    @DisplayName("scan() returns COMPLETE for empty block range")
    ...
    @Test
    @DisplayName("scan() with no matching events returns 0 events, COMPLETE")
    ...
    @Test
    @DisplayName("scan() decodes and delivers Transfer events to handler")
    ...
    @Test
    @DisplayName("scan() aggregates events across multiple chunks")
    ...
    @Test
    @DisplayName("scan() stops early when handler returns false")
    ...
    @Test
    @DisplayName("collect() returns all events as a list")
    ...
    @Test
    @DisplayName("collect() with no events returns empty list")
    ...
    @Test
    @DisplayName("count() returns total number of events")
    ...
    @Test
    @DisplayName("count() with no events returns 0")
    ...
    @Test
    @DisplayName("ScanProgress percentDone increases chunk to chunk")
    ...
    @Test
    @DisplayName("ScanProgress contains correct block range fields")
    ...
    @Test
    @DisplayName("ScanProgress.cursor() returns next block after chunk")
    ...
    @Test
    @DisplayName("ScanProgress.toString() is human-readable")
    ...
    @Test
    @DisplayName("scan() with cursor starts from cursor.nextBlock()")
    ...
    @Test
    @DisplayName("ScanResult.cursor() = lastBlock + 1")
    void result_cursor() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("[]");
            var result =
                    BlockScanner.of(rpc.client(), 2000)
                            .scan(USDC, TRANSFER, 0L, 999L, (e, p) -> true)
                            .join();
    ...
    @Test
    @DisplayName("ScanResult.isComplete() true when fully scanned")
    void result_is_complete() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("[]");
            var result =
                    BlockScanner.of(rpc.client(), 2000)
                            .scan(USDC, TRANSFER, 0L, 100L, (e, p) -> true)
                            .join();
    ...
    @Test
    @DisplayName("ScanResult.isEarlyExit() true when handler returned false")
    void result_is_early_exit() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("[]");
            var result =
                    BlockScanner.of(rpc.client(), 2000)
                            .scan(USDC, TRANSFER, 0L, 100L, (e, p) -> false)
                            .join();
    ...
    @Test
    @DisplayName("ScanResult.toString() is human-readable")
        ...
        try (var rpc = new RpcMock()) {
            ...
            var result =
                    BlockScanner.of(rpc.client(), 2000)
                            ...
                            .join();
            assertTrue(
                    result.toString().contains("events") || result.toString().contains("COMPLETE"),
                    result.toString());
        }
    }
    ...
    @Test
    @DisplayName("BlockScanner.of(client) uses default chunk size 2000")
    ...
    @Test
    @DisplayName("BlockScanner.of(client, 500) uses custom chunk size")
    ...
    @Test
    @DisplayName("scanMulti() with empty contracts/events returns COMPLETE immediately")
    void scan_multi_empty() throws Exception {
        try (var rpc = new RpcMock()) {
            var result =
                    BlockScanner.of(rpc.client())
                            .scanMulti(List.of(), List.of(), 0L, 100L, (e, p) -> true)
                            .join();
            assertEquals(BlockScanner.ScanResult.Reason.COMPLETE, result.reason());
    ...
    @Test
    @DisplayName("scanMulti() merges events from multiple contracts")
