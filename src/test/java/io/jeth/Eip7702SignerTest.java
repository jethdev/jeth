 ...
 *
 * <p>Verifies: authorization signing, type byte, structure, self-sponsoring, any-chain
 * authorizations, and the full sign() round-trip.
 */
class Eip7702SignerTest {
    ...
    static final Wallet WALLET =
            Wallet.fromPrivateKey(new BigInteger(PRIVATE_KEY.substring(2), 16));
    ...
    @Test
    @DisplayName("signAuthorization() returns non-null Authorization")
    ...
    @Test
    @DisplayName("Authorization has correct chainId")
    void auth_chain_id() {
        var auth = Eip7702Signer.signAuthorization(1L, CONTRACT, 0L, WALLET);
    ...
    @Test
    @DisplayName("Authorization has correct contract address")
    ...
    @Test
    @DisplayName("Authorization has correct nonce")
    void auth_nonce() {
    ...
    @Test
    @DisplayName("Authorization yParity is 0 or 1")
    void auth_y_parity() {
        var auth = Eip7702Signer.signAuthorization(1L, CONTRACT, 0L, WALLET);
        assertTrue(
    ...
    @Test
    @DisplayName("Authorization r and s are non-empty hex strings")
    ...
    @Test
    @DisplayName("Authorization r and s fit in 32 bytes")
    void auth_r_s_size() {
        var auth = Eip7702Signer.signAuthorization(1L, CONTRACT, 0L, WALLET);
    ...
    @Test
    @DisplayName("Same inputs produce identical authorizations (deterministic ECDSA via RFC6979)")
    ...
    @Test
    @DisplayName("Different chainIds produce different signatures")
    ...
    @Test
    @DisplayName("Authorization.anyChain() has chainId = 0")
    ...
    @Test
    @DisplayName("Authorization.selfSponsored() is equivalent to signAuthorization")
    ...
    @Test
    @DisplayName("sign() returns 0x-prefixed hex string")
    ...
    @Test
    @DisplayName("sign() produces type-4 transaction (first byte = 0x04)")
    ...
    @Test
    @DisplayName("sign() with multiple authorizations is longer than with one")
    void sign_multiple_auths() {
        ...
        assertTrue(two.length() > one.length(), "Two authorizations must produce longer raw tx");
    }

    @Test
    @DisplayName("sign() with zero-value tx and no calldata doesn't throw")
    void sign_minimal() {
        var auth = Eip7702Signer.Authorization.anyChain(CONTRACT, 0L, WALLET);
        var tx =
                EthModels.TransactionRequest.builder()
    ...
    @Test
    @DisplayName("Authorization.toString() is human-readable")
    void to_string() {
        var auth = Eip7702Signer.signAuthorization(1L, CONTRACT, 0L, WALLET);
        String s = auth.toString();
        assertTrue(
                s.contains("chain=1") || s.contains("chainId=1") || s.contains("Authorization"),
