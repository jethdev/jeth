...
class MulticallTest {
    ...
    @Test @DisplayName("ADDRESS constant is the canonical Multicall3 address")
    void canonical_address() {
        assertEquals("0xcA11bde05977b3631167028862bE2a173976CA11", Multicall3.ADDRESS);
    }

    @Test @DisplayName("empty execute() makes zero RPC calls")
    ...
    @Test @DisplayName("size() tracks added calls correctly")
    ...
    @Test @DisplayName("add() returns sequential indexes 0, 1, 2...")
    ...
    @Test @DisplayName("aggregate3 selector 0x82ad56cb is in calldata")
    void aggregate3_selector_in_calldata() throws Exception {
        try (var rpc = new RpcMock()) {
            // Build a fake aggregate3 response for 1 call returning uint256(1)
            rpc.enqueue(buildAggregate3Response(
                new byte[][]{padUint256(1)}, new boolean[]{true}));
            var mc = new Multicall3(rpc.client());
            Function fn = Function.of("balanceOf", AbiType.ADDRESS).withReturns(AbiType.UINT256);
    ...
    @Test @DisplayName("decode: successful call returns decoded value")
    void decode_success() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueue(buildAggregate3Response(new byte[][]{padUint256(42)}, new boolean[]{true}));
            var mc = new Multicall3(rpc.client());
            Function fn = Function.of("balanceOf", AbiType.ADDRESS).withReturns(AbiType.UINT256);
            mc.add("0xToken", fn, "0xUser");
            List<Object> results = mc.execute().join();
    ...
    @Test @DisplayName("addOptional: failed call returns null (no throw)")
    ...
    static String buildAggregate3Response(byte[][] results, boolean[] success) {
        ...
        for (int i = 0; i < n; i++) { writeInt(outer, 64 + i*32 + 28, off); off += elements[i].length; }
        int pos = 64 + n * 32;
        for (byte[] e : elements) { System.arraycopy(e, 0, outer, pos, e.length); pos += e.length; }
        return "\"0x" + io.jeth.util.Hex.encodeNoPrefx(outer) + "\"";
    }

    static byte[] padUint256(long v) {
        byte[] b = new byte[32]; b[31] = (byte)v; b[30] = (byte)(v>>8); return b;
    }
    static void writeInt(byte[] buf, int pos, int v) {
        buf[pos]=(byte)(v>>24); buf[pos+1]=(byte)(v>>16); buf[pos+2]=(byte)(v>>8); buf[pos+3]=(byte)v;
    }
    static int totalLen(byte[][] arrs) { int t=0; for(byte[] a:arrs) t+=a.length; return t; }
}
