 ...
 *
 * <p>Verifies: function keyword, param names, return types, modifiers, selectors, tuple types, and
 * edge cases.
 */
class HumanAbiTest {
    ...
    @Test
    @DisplayName("Parse 'function transfer(address,uint256)'")
    ...
    @Test
    @DisplayName("Parse with named params strips names")
    ...
    @Test
    @DisplayName("Shorthand (no 'function' keyword) is accepted")
    void parse_shorthand() {
        Function fn = HumanAbi.parseFunction("transfer(address,uint256)");
        assertEquals("transfer", fn.getName());
        assertEquals(
                HumanAbi.parseFunction("function transfer(address,uint256)").getSelectorHex(),
                fn.getSelectorHex());
    }

    @Test
    @DisplayName("No-arg function parses correctly")
    void parse_no_args() {
    ...
    @Test
    @DisplayName("transfer(address,uint256) → 0xa9059cbb")
    ...
    @Test
    @DisplayName("balanceOf(address) → 0x70a08231")
    void selector_balance_of() {
        assertEquals(
                "0x70a08231", HumanAbi.parseFunction("function balanceOf(address)").selector());
    }

    @Test
    @DisplayName("approve(address,uint256) → 0x095ea7b3")
    void selector_approve() {
        assertEquals("0x095ea7b3", HumanAbi.parseFunction("approve(address,uint256)").selector());
    }

    @Test
    @DisplayName("HumanAbi.selector() convenience matches parseFunction().selector()")
    ...
    @Test
    @DisplayName("'returns (bool)' sets output type")
    void returns_bool() {
    ...
    @Test
    @DisplayName("'view returns (uint256)' strips 'view' and parses return type")
    ...
    @Test
    @DisplayName("'pure returns (bytes32)' strips 'pure'")
    ...
    @Test
    @DisplayName("No 'returns' clause gives no output types or empty")
    void no_returns() {
    ...
    @Test
    @DisplayName("Multiple return types: returns (address, uint256)")
    ...
    @Test
    @DisplayName("'external view' modifier is stripped correctly")
    ...
    @Test
    @DisplayName("'payable' modifier doesn't affect selector")
    ...
    @Test
    @DisplayName("'nonpayable' modifier doesn't affect selector")
    ...
    @Test
    @DisplayName("parseFunctions() parses 3-function ERC-20 subset")
    void parse_functions_erc20() {
        List<Function> fns =
                HumanAbi.parseFunctions(
                        """
    ...
    @Test
    @DisplayName("parseFunctions() skips comment lines (//)")
    ...
    @Test
    @DisplayName("parseFunctions() skips blank lines")
    ...
    @Test
    @DisplayName("parseFunctions() with empty string returns empty list")
    ...
    @Test
    @DisplayName("Fragment without '(' throws IllegalArgumentException")
    void error_no_paren() {
        assertThrows(
                IllegalArgumentException.class, () -> HumanAbi.parseFunction("function transfer"));
    }

    @Test
    @DisplayName("Fragment with empty name throws")
    void error_empty_name() {
        assertThrows(
                IllegalArgumentException.class, () -> HumanAbi.parseFunction("function (address)"));
    }

    // ─── Internal helpers (accessible via package) ────────────────────────────

    @Test
    @DisplayName("splitParams splits 'a,b,c' into 3 items")
    ...
    @Test
    @DisplayName("splitParams respects nested parens: '(a,b),c' → 2 items")
    ...
    @Test
    @DisplayName("extractType strips param name: 'address to' → 'address'")
    void extract_type_strips_name() {
        assertEquals("address", HumanAbi.extractType("address to"));
    }

    @Test
    @DisplayName("extractType handles 'indexed' modifier for events")
