...
class EventDecoderTest {
    ...
    static final String TRANSFER_SIG =
    ...
    static final EventDecoder APPROVAL =
            EventDecoder.of(
    ...
    @Test
    @DisplayName("getTopic0() starts with exactly one '0x' prefix — not '0x0x'")
    ...
    @Test
    @DisplayName("getTopic0() is 66 characters (0x + 64 hex chars = 32 bytes)")
    void topic0_length() {
        assertEquals(66, TRANSFER.getTopic0().length());
    }

    @Test
    @DisplayName("getTopic0() matches known ERC-20 Transfer hash")
    void topic0_known_transfer_hash() {
        assertEquals(TRANSFER_T0, TRANSFER.getTopic0());
    }

    @Test
    @DisplayName("getTopic0() matches manual keccak256 of normalized signature")
    ...
    @Test
    @DisplayName("Static ERC20_TRANSFER constant has correct topic0")
    void erc20_transfer_constant() {
        assertEquals(TRANSFER_T0, EventDecoder.ERC20_TRANSFER.getTopic0());
    }

    @Test
    @DisplayName("Static ERC20_APPROVAL constant has 0x-prefixed topic0")
    ...
    @Test
    @DisplayName("Static ERC721_TRANSFER constant has 0x-prefixed topic0")
    ...
    @Test
    @DisplayName("matches() true when log topic0 equals event topic0")
    ...
    @Test
    @DisplayName("matches() false for wrong event")
    ...
    @Test
    @DisplayName("matches() false for log with no topics")
    ...
    @Test
    @DisplayName("matches() false for null topics")
    ...
    @Test
    @DisplayName("matches() is case-insensitive for topic0 comparison")
    void matches_case_insensitive() {
        String upperT0 = TRANSFER_T0.toUpperCase();
        assertTrue(
                TRANSFER.matches(
                        buildLog(upperT0, padAddr(ADDR_FROM), padAddr(ADDR_TO), encodeUint(1))));
    }

    // ─── decode() ─────────────────────────────────────────────────────────────

    @Test
    @DisplayName("decode() extracts indexed 'from' address")
    ...
    @Test
    @DisplayName("decode() extracts indexed 'to' address")
    ...
    @Test
    @DisplayName("decode() extracts non-indexed uint256 value from data field")
    void decode_value() {
    ...
    @Test
    @DisplayName("decode() throws when log does not match")
    void decode_throws_on_mismatch() {
        var log =
                buildLog(APPROVAL.getTopic0(), padAddr(ADDR_FROM), padAddr(ADDR_TO), encodeUint(0));
        assertThrows(IllegalArgumentException.class, () -> TRANSFER.decode(log));
    }

    @Test
    @DisplayName("decode() handles all three indexed params (ERC-721 Transfer)")
    void decode_all_indexed() {
        ...
        var log =
                buildLog(
                        erc721.getTopic0(), padAddr(ADDR_FROM), padAddr(ADDR_TO), padUint(tokenId));
    ...
    @Test
    @DisplayName("decodeAll() returns only matching logs")
    void decode_all_filters() {
        var t1 = buildLog(TRANSFER_T0, padAddr(ADDR_FROM), padAddr(ADDR_TO), encodeUint(1));
        var ap =
                buildLog(APPROVAL.getTopic0(), padAddr(ADDR_FROM), padAddr(ADDR_TO), encodeUint(2));
    ...
    @Test
    @DisplayName("decodeAll() on empty list returns empty")
    ...
    @Test
    @DisplayName("Verbose and minimal signatures produce same topic0")
    void signature_normalization() {
        EventDecoder verbose =
                EventDecoder.of(
                        "Transfer(address indexed from, address indexed to, uint256 value)");
        EventDecoder minimal = EventDecoder.of("Transfer(address,address,uint256)");
        assertEquals(verbose.getTopic0(), minimal.getTopic0());
    }

    @Test
    @DisplayName("getSignature() returns canonical (no param names)")
    ...
    static String encodeUint(long v) {
        return encodeUint(BigInteger.valueOf(v));
    }
    ...
    static String padUint(BigInteger v) {
        return encodeUint(v);
    }
}
