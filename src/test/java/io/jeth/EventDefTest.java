...
class EventDefTest {
    ...
    static final EventDef APPROVAL = EventDef.of("Approval",
    ...
    @Test @DisplayName("Transfer topic0 = keccak256 of signature")
    void topic0_keccak() {
    ...
    @Test @DisplayName("topic0 matches known ERC-20 Transfer value")
    ...
    @Test @DisplayName("getTopic0() returns 32 bytes")
    ...
    @Test @DisplayName("matches() true when topic0 matches")
    void matches_true() {
        assertNotNull(TRANSFER.decode(buildLog(TRANSFER_T0, pad(ADDR_FROM), pad(ADDR_TO), encodeU256(1000))));
    }

    @Test @DisplayName("matches() false when topic0 is different event")
    void matches_false() {
        assertNull(TRANSFER.decode(buildLog(APPROVAL.topic0Hex(), pad(ADDR_FROM), pad(ADDR_TO), encodeU256(1000))));
    }

    // ── decode() ─────────────────────────────────────────────────────────────

    @Test @DisplayName("decode() extracts indexed address params")
    void decode_addresses() {
        ...
        assertTrue(ADDR_FROM.equalsIgnoreCase(decoded.get("from").toString()), "from: " + decoded.get("from"));
        assertTrue(ADDR_TO.equalsIgnoreCase(decoded.get("to").toString()),     "to: "   + decoded.get("to"));
    }

    @Test @DisplayName("decode() extracts uint256 data field")
    void decode_uint256_value() {
        BigInteger expected = new BigInteger("1000000000000000000");
        var log = buildLog(TRANSFER_T0, pad(ADDR_FROM), pad(ADDR_TO), encodeU256(expected.longValue()));
        assertEquals(expected, TRANSFER.decode(log).get("value"));
    }

    @Test @DisplayName("decode() returns null for non-matching log")
    ...
    @Test @DisplayName("decoded.address() helper returns string")
    ...
    @Test @DisplayName("decoded.uint() helper returns BigInteger")
    ...
    @Test @DisplayName("decodeAll() filters only matching logs")
    ...
    @Test @DisplayName("decodeAll() on empty log list returns empty")
    ...
    static String encodeU256(long v) {
        byte[] b = AbiCodec.encode(new AbiType[]{AbiType.UINT256}, new Object[]{BigInteger.valueOf(v)});
        return Hex.encode(b);
    }
}
