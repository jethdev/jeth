...
class TypeMapperAndAbiJsonTest {
    ...
    class TypeMapperTests {
        ...
        @Test @DisplayName("toJavaTypeBoxed: bool → Boolean (boxed)")
        void bool_boxed() {
            String t = TypeMapper.toJavaTypeBoxed("bool");
            assertEquals("Boolean", t);
        }

        @Test @DisplayName("toJavaTypeBoxed: uint256 → BigInteger (already boxed)")
        void uint256_boxed() {
            assertEquals("BigInteger", TypeMapper.toJavaTypeBoxed("uint256"));
        }

        @Test @DisplayName("importsFor: uint256 needs BigInteger import")
        void imports_uint256() {
            List<String> imports = TypeMapper.importsFor("uint256");
            assertTrue(imports.stream().anyMatch(i -> i.contains("BigInteger")),
                "uint256 must require BigInteger import");
        }

        @Test @DisplayName("importsFor: address needs no extra imports")
        void imports_address() {
            ...
            assertTrue(imports.isEmpty() || imports.stream().noneMatch(i -> i.contains("BigInteger")));
        }

        @Test @DisplayName("toAbiTypeExpr: uint256 → AbiType.UINT256")
        void abi_type_expr_uint256() {
            String expr = TypeMapper.toAbiTypeExpr("uint256");
            assertTrue(expr.contains("UINT256") || expr.contains("uint256"),
                "uint256 ABI type expression should reference UINT256, got: " + expr);
        }

        @Test @DisplayName("toAbiTypeExpr: address → AbiType.ADDRESS")
        void abi_type_expr_address() {
            String expr = TypeMapper.toAbiTypeExpr("address");
            assertTrue(expr.contains("ADDRESS") || expr.contains("address"),
                "address ABI type expression should reference ADDRESS, got: " + expr);
        }

        @Test @DisplayName("castFrom: address casts to String")
        ...
        @Test @DisplayName("castFrom: uint256 casts to BigInteger")
    ...
    class AbiJsonTests {
        ...
        @Test @DisplayName("parse() returns correct entry count")
        void parse_entry_count() {
            List<AbiJson.Entry> entries = AbiJson.parse(ERC20_ABI);
            assertEquals(4, entries.size());
        }

        @Test @DisplayName("parse() identifies function entries")
        void parse_functions() {
            List<AbiJson.Entry> fns = AbiJson.parse(ERC20_ABI).stream()
                .filter(e -> "function".equals(e.type)).toList();
            assertEquals(2, fns.size());
        }

        @Test @DisplayName("parse() identifies event entries")
        void parse_events() {
            List<AbiJson.Entry> events = AbiJson.parse(ERC20_ABI).stream()
                .filter(e -> "event".equals(e.type)).toList();
            assertEquals(1, events.size());
            assertEquals("Transfer", events.get(0).name);
        }

        @Test @DisplayName("parse() identifies constructor entry")
        void parse_constructor() {
            List<AbiJson.Entry> ctors = AbiJson.parse(ERC20_ABI).stream()
                .filter(e -> "constructor".equals(e.type)).toList();
            assertEquals(1, ctors.size());
        }

        @Test @DisplayName("balanceOf has one address input")
        void balanceof_inputs() {
            AbiJson.Entry balanceOf = AbiJson.parse(ERC20_ABI).stream()
                .filter(e -> "balanceOf".equals(e.name)).findFirst().orElseThrow();
            assertEquals(1, balanceOf.inputs.size());
            assertEquals("address", balanceOf.inputs.get(0).type);
        }

        @Test @DisplayName("transfer has two inputs: address and uint256")
        void transfer_inputs() {
            AbiJson.Entry transfer = AbiJson.parse(ERC20_ABI).stream()
        ...
        @Test @DisplayName("Transfer event has 3 inputs")
        void transfer_event_inputs() {
            AbiJson.Entry ev = AbiJson.parse(ERC20_ABI).stream()
                .filter(e -> "Transfer".equals(e.name)).findFirst().orElseThrow();
        ...
        @Test @DisplayName("parse() empty ABI returns empty list")
        void parse_empty() {
            assertTrue(AbiJson.parse("[]").isEmpty());
        }

        @Test @DisplayName("view function stateMutability is preserved")
        ...
        @Test @DisplayName("nonpayable function stateMutability is preserved")
