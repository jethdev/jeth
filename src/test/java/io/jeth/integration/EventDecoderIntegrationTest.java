...
/**
 * Integration tests for {@link EventDecoder} and {@link EventDef} using real logs
 * from a deployed TestToken contract on Besu dev network.
 *
 * <p>Key regression: EventDecoder.getTopic0() must produce "0x..." not "0x0x..."
 * (double-prefix bug). These tests verify the decoder works with REAL node-returned
 * logs — if topic0 were double-prefixed, {@code matches()} would never return true.
 *
...
class EventDecoderIntegrationTest {
    ...
    static final EventDecoder TRANSFER_DECODER = EventDecoder.of(
            "Transfer(address indexed from, address indexed to, uint256 value)");

    static final EventDecoder APPROVAL_DECODER = EventDecoder.of(
            "Approval(address indexed owner, address indexed spender, uint256 value)");

    static final EventDef TRANSFER_DEF = EventDef.of("Transfer",
    ...
    @Test @Order(0) @DisplayName("EventDecoder.getTopic0() has exactly one '0x' prefix (regression)")
    ...
    @Test @Order(1) @DisplayName("EventDecoder.getTopic0() matches known Transfer hash")
    void topic0_value_correct() {
        assertEquals(TRANSFER_T0, TRANSFER_DECODER.getTopic0());
    }

    @Test @Order(2) @DisplayName("EventDef.topic0Hex() has exactly one '0x' prefix (regression)")
    ...
    @Test @Order(10) @DisplayName("Constructor emits Transfer(0x0 → deployer, totalSupply)")
    void constructor_mint_log_matches() {
        List<EthModels.Log> logs = client.getLogs("0x0", "latest", tokenAddr,
                List.of(TRANSFER_T0)).join();
        ...
        assertTrue(TRANSFER_DECODER.matches(mintLog),
                "EventDecoder.matches() must return true for real Transfer log; " +
                "failure = topic0 format bug (double-prefix)");
    }

    @Test @Order(11) @DisplayName("Constructor mint: decoded value = totalSupply (1_000_000 TT)")
    ...
    @Test @Order(12) @DisplayName("Constructor mint: from = 0x0 (mint from zero address)")
    void constructor_mint_log_from_zero() {
        ...
        assertTrue(from.replaceAll("[0x]", "").isBlank() || from.equals("0x0000000000000000000000000000000000000000"),
                "from must be zero address for mint log, got: " + from);
    }

    // ─── transfer() logs ──────────────────────────────────────────────────────

    @Test @Order(20) @DisplayName("matches() returns true for Transfer log from real transfer()")
    void matches_real_transfer_log() throws Exception {
        var recipient = Wallet.create();
        var receipt = doTransfer(recipient, Units.toWei("1"));

        EthModels.Log log = receipt.logs.stream()
                .filter(TRANSFER_DECODER::matches)
                .findFirst().orElseThrow(() -> new AssertionError("No Transfer log"));

        assertTrue(TRANSFER_DECODER.matches(log));
    }

    @Test @Order(21) @DisplayName("decode() extracts correct 'from' address")
    void decode_from_address() throws Exception {
        var recipient = Wallet.create();
        var receipt   = doTransfer(recipient, Units.toWei("2"));

        EthModels.Log log = receipt.logs.stream().filter(TRANSFER_DECODER::matches).findFirst()
                .orElseThrow(() -> new AssertionError("No Transfer log"));

        var decoded = TRANSFER_DECODER.decode(log);
        assertTrue(dev.getAddress().equalsIgnoreCase((String) decoded.get("from")),
                "Decoded 'from' must be the sender");
    }

    @Test @Order(22) @DisplayName("decode() extracts correct 'to' address")
    void decode_to_address() throws Exception {
        ...
        var decoded = TRANSFER_DECODER.decode(log);
        assertTrue(recipient.getAddress().equalsIgnoreCase((String) decoded.get("to")),
                "Decoded 'to' must be the recipient");
    }

    @Test @Order(23) @DisplayName("decode() extracts correct 'value'")
    ...
    @Test @Order(24) @DisplayName("decodeAll() returns only Transfer logs from receipt")
    ...
    @Test @Order(25) @DisplayName("EventDecoder and EventDef decode value consistently")
    ...
    @Test @Order(26) @DisplayName("approve() emits Approval log — decoded separately from Transfer")
    void approval_log_decoded() throws Exception {
        Wallet spender = Wallet.create();
        String txHash  = token.fn("approve(address,uint256)").send(dev, spender.getAddress(), Units.toWei("500")).join();
        var    receipt = client.waitForTransaction(txHash, 30_000, 500).join();

        // Approval log should match APPROVAL but not TRANSFER
        EthModels.Log approvalLog = receipt.logs.stream()
                .filter(APPROVAL_DECODER::matches).findFirst()
                .orElseThrow(() -> new AssertionError("No Approval log"));

        assertFalse(TRANSFER_DECODER.matches(approvalLog), "Approval log must not match Transfer decoder");
    ...
    @Test @Order(27) @DisplayName("getLogs() with topic0 filter returns only Transfer logs")
    void get_logs_filtered() throws Exception {
        ...
        List<EthModels.Log> logs = client.getLogs(
        ...
        assertTrue(logs.stream().allMatch(TRANSFER_DECODER::matches),
                "All getLogs results must match Transfer topic0");
    }

    // ─── EventDef (legacy API consistency) ────────────────────────────────────

    @Test @Order(30) @DisplayName("EventDef.decode() works on real Transfer log")
    void event_def_decode_real_log() throws Exception {
        BigInteger amount  = Units.toWei("5");
        var        receipt = doTransfer(Wallet.create(), amount);

        EthModels.Log log = receipt.logs.stream()
                .filter(l -> l.topics != null && !l.topics.isEmpty()
    ...
    EthModels.TransactionReceipt doTransfer(Wallet recipient, BigInteger amount) throws Exception {
        String txHash = token.fn("transfer(address,uint256)")
                .send(dev, recipient.getAddress(), amount).join();
