...
class WsProviderIntegrationTest {
    ...
    @Test @DisplayName("connect() establishes WS connection and provides a provider")
    @Timeout(15)
    ...
    @Test @DisplayName("connect() with builder allows custom settings")
    @Timeout(15)
    void connect_via_builder() {
        ws = WsProvider.builder(besu.wsUrl())
    ...
    @Test @DisplayName("onNewBlock() receives block headers when blocks are mined")
    @Timeout(30)
    void on_new_block_receives_headers() throws Exception {
        ...
        String subId = ws.onNewBlock(header -> {
    ...
    @Test @DisplayName("onNewBlock() subscription has a valid 0x-prefixed sub ID")
    @Timeout(15)
    ...
    @Test @DisplayName("onNewBlock() headers contain expected fields")
    @Timeout(30)
    void on_new_block_header_fields() throws Exception {
        ...
        ws.onNewBlock(header -> { blockRef.set(header); latch.countDown(); }).join();
        IntegrationTestBase.sendEth(http, dev, Wallet.create().getAddress(), Units.toWei("0.001"));
    ...
    @Test @DisplayName("Multiple onNewBlock() subscriptions both fire independently")
    @Timeout(30)
    ...
    @Test @DisplayName("onPendingTransaction() receives tx hash before block mines")
    @Timeout(30)
    ...
    @Test @DisplayName("unsubscribe() returns true for a valid subscription")
    @Timeout(15)
    ...
    @Test @DisplayName("unsubscribe() stops the handler from firing on subsequent blocks")
    @Timeout(30)
    void unsubscribe_stops_handler() throws Exception {
        ...
        assertEquals(countAfterUnsub, fireCount.get(),
    ...
    @Test @DisplayName("RPC calls still work over WebSocket (not just subscriptions)")
    @Timeout(15)
    ...
    @Test @DisplayName("WS and HTTP providers return consistent chainId")
    @Timeout(15)
