...
/**
 * Integration tests for {@link EventSubscriber} and {@link ContractEvents}
 * — subscribe to real contract logs over WebSocket.
 *
 * <p>Uses TestCounter (simpler, fewer setup dependencies) for increment/event tests
 * and TestToken for Transfer log tests.
 *
...
class EventSubscriberIntegrationTest {
    ...
    @Test @Order(1) @DisplayName("ContractEvents.on() receives Incremented event from real tx")
    @Timeout(30)
    void contract_events_on_incremented() throws Exception {
        EventDef Incremented = EventDef.of("Incremented",
        ...
        assertTrue(latch.await(20, TimeUnit.SECONDS), "Should receive Incremented event within 20s");
    ...
    @Test @Order(2) @DisplayName("ContractEvents.on() receives ValueSet event with correct values")
    @Timeout(30)
    void contract_events_on_value_set() throws Exception {
        EventDef ValueSet = EventDef.of("ValueSet",
        ...
        assertTrue(dev.getAddress().equalsIgnoreCase(receivedWho.get()),
                "Received 'who' must match sender");
    }

    @Test @Order(3) @DisplayName("ContractEvents.on() receives Transfer log from TestToken")
    @Timeout(30)
    void contract_events_on_transfer() throws Exception {
        EventDef Transfer = EventDef.of("Transfer",
        ...
        String txHash = token.fn("transfer(address,uint256)").send(dev, recipient.getAddress(), amount).join();
        client.waitForTransaction(txHash, 30_000, 500).join();
        ...
        assertTrue(recipient.getAddress().equalsIgnoreCase(receivedTo.get()),
                "Received 'to' must match recipient");
    }

    @Test @Order(4) @DisplayName("ContractEvents.once() fires exactly once then stops")
    @Timeout(30)
    ...
    @Test @Order(5) @DisplayName("Two ContractEvents subscriptions on same contract are independent")
    @Timeout(30)
    ...
    @Test @Order(10) @DisplayName("EventSubscriber.on() receives Transfer events (deprecated API)")
    @Timeout(30)
    void event_subscriber_on_transfer() throws Exception {
        ...
        subscriber.on(Transfer, tokenAddr, decoded -> {
            receivedAmount.set(decoded.uint("value"));
            latch.countDown();
        });
    ...
    @Test @Order(11) @DisplayName("EventSubscriber.off() stops receiving events")
    @Timeout(30)
    void event_subscriber_off_stops_events() throws Exception {
        ...
        EventSubscriber subscriber = EventSubscriber.of(ws);
        String subId = subscriber.on(Incremented, null, decoded -> {
    ...
    @Test @Order(20) @DisplayName("Contract-specific subscription does not receive events from other contract")
    @Timeout(30)
    void subscription_is_contract_specific() throws Exception {
        ...
        String counter2Addr = IntegrationTestBase.deploy(client, dev, SolcContainer.compile("TestCounter"));
        ...
        ContractEvents events = new ContractEvents(counterAddr, ws);
        events.on(Incremented, d -> { fireCount.incrementAndGet(); expectedFire.countDown(); });
        ...
        assertEquals(0, fireCount.get(), "Subscription must not fire for events from another contract");

        // Increment counter (SHOULD trigger)
        String txHash = counter.fn("increment()").send(dev).join();
        client.waitForTransaction(txHash, 30_000, 500).join();

        assertTrue(expectedFire.await(20, TimeUnit.SECONDS), "Should fire for own contract's event");
