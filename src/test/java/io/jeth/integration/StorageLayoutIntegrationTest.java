...
/**
 * Integration tests for {@link StorageLayout} — reads real on-chain storage slots from a deployed
 * TestCounter contract on Besu dev network.
 *
...
class StorageLayoutIntegrationTest {
    ...
    @Test
    @Order(0)
    @DisplayName("readSlot(0) on EOA = 0")
    ...
    @Test
    @Order(1)
    @DisplayName("slot 0 (count) = 0 after deploy")
    ...
    @Test
    @Order(2)
    @DisplayName("slot 0 (count) = 1 after increment()")
    ...
    @Test
    @Order(3)
    @DisplayName("slot 0 (count) = 6 after incrementBy(5)")
    ...
    @Test
    @Order(4)
    @DisplayName("slot 0 matches view function count()")
    void slot_matches_view_call() {
        BigInteger fromStorage = storage.readSlot(0L).join();
        BigInteger fromCall =
                counter.fn("count()").returns("uint256").call().as(BigInteger.class).join();
    ...
    @Test
    @Order(10)
    @DisplayName("slot 1 raw is non-zero (contains owner + initialized)")
    void slot1_nonzero() {
    ...
    @Test
    @Order(11)
    @DisplayName("slot 1: unpack owner address (bytes 0-19)")
    ...
    @Test
    @Order(12)
    @DisplayName("slot 1: unpack initialized bool (byte 20) = 1 (true)")
    ...
    @Test
    @Order(13)
    @DisplayName("slot 1 owner bytes match owner() view call")
    void slot1_owner_matches_view() {
        BigInteger raw = storage.readSlot(1L).join();
        BigInteger fromSlot = StorageLayout.unpackSlot(raw, 0, 20);
        String fromCall = counter.fn("owner()").returns("address").call().as(String.class).join();
    ...
    @Test
    @Order(20)
    @DisplayName("values[dev] = 0 initially (unset mapping entry)")
    ...
    @Test
    @Order(21)
    @DisplayName("values[dev] = 42 after setValue(42)")
    ...
    @Test
    @Order(22)
    @DisplayName("values[unknown] = 0 (different key, unset)")
    void mapping_zero_for_different_key() {
        assertEquals(BigInteger.ZERO, storage.readMapping(2L, Wallet.create().getAddress()).join());
    }

    @Test
    @Order(23)
    @DisplayName("mapping slot matches values() view call")
    void mapping_slot_matches_view() {
        BigInteger fromStorage = storage.readMapping(2L, dev.getAddress()).join();
        BigInteger fromCall =
                counter.fn("values(address)")
                        .returns("uint256")
                        .call(dev.getAddress())
                        .as(BigInteger.class)
                        .join();
    ...
    @Test
    @Order(30)
    @DisplayName("readSlotBytes() returns 0x-prefixed hex string")
