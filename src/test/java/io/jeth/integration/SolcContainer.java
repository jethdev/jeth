...
/**
 * Compiles Solidity contracts using the {@code ethereum/solc:0.8.20} Docker image,
 * eliminating the need for a local {@code solc} installation or environment variables.
 *
 * <p>A single shared container instance is started once per JVM run, stays alive
 * via a {@code tail -f /dev/null} entrypoint, and each compile is an {@code execInContainer}
 * call. Results are cached so contracts are only compiled once.
 *
 * <p>Usage:
 * <pre>
...
public class SolcContainer {
    ...
    private static final GenericContainer<?> CONTAINER =
        new GenericContainer<>(DockerImageName.parse(SOLC_IMAGE))
            .withCreateContainerCmdModifier(cmd ->
                // Override entrypoint so the container stays alive for exec calls
    ...
    /**
     * Compile all known test contracts and return name -> bytecode map.
     * Safe to call multiple times — cached after first run.
     */
    ...
    /**
     * Compile the named contract (filename = &lt;name&gt;.sol).
     * Returns 0x-prefixed deployment bytecode. Cached after first compile.
     */
    ...
    private static String doCompile(String contractName) {
        ...
        if (resource == null) {
            throw new IllegalStateException(
                "Cannot find /contracts/" + fileName + " in classpath");
        }
        ...
        try {
            ...
            if (result.getExitCode() != 0) {
                throw new RuntimeException("solc failed (exit " + result.getExitCode()
                    + "):\nstderr: " + result.getStderr());
            }

            String bytecode = extractBytecode(result.getStdout(), contractName);
            System.out.println("[SolcContainer] " + contractName + " compiled: "
                + (bytecode.length() / 2 - 1) + " bytes");
            return bytecode;
