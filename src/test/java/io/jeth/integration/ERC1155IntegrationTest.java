 ...
 *
 * <p>TestERC1155 is compiled at test time via {@link SolcContainer} — no env vars needed.
 * The test contract has a {@code mint()} function (callable by anyone) for easy test setup.
 *
 * Run: {@code ./gradlew integrationTest}
 */
@Tag("integration")
@Testcontainers
...
class ERC1155IntegrationTest {
    ...
    @Test @Order(1) @DisplayName("getAddress() returns the contract address")
    void get_address() {
        ERC1155 token = new ERC1155(contractAddr, client);
        assertEquals(contractAddr, token.getAddress());
    }

    @Test @Order(2) @DisplayName("balanceOf() returns 0 before any mint")
    ...
    @Test @Order(3) @DisplayName("uri() returns the token URI set in constructor")
    void uri_matches_constructor() {
        ERC1155 token = new ERC1155(contractAddr, client);
        String uri = token.uri(TOKEN_ID_1).join();
        assertNotNull(uri);
        assertFalse(uri.isBlank(), "uri must not be blank");
        assertTrue(uri.contains("api.test") || uri.contains("{id}"),
            "uri must contain expected value, got: " + uri);
    }

    @Test @Order(4) @DisplayName("isApprovedForAll() returns false before setApprovalForAll")
    ...
    @Test @Order(10) @DisplayName("balanceOf() returns correct amount after mint")
    void balance_after_mint() throws Exception {
        ...
        ERC1155    token = new ERC1155(contractAddr, client);
        BigInteger bal   = token.balanceOf(dev.getAddress(), TOKEN_ID_1).join();
        assertTrue(bal.compareTo(amount) >= 0,
            "Balance must be >= " + amount + " after minting, got " + bal);
    }

    // ─── setApprovalForAll ────────────────────────────────────────────────────

    @Test @Order(20) @DisplayName("setApprovalForAll() grants approval — isApprovedForAll() returns true")
    ...
    @Test @Order(21) @DisplayName("setApprovalForAll(false) revokes approval")
    void revoke_approval() throws Exception {
        ...
        client.waitForTransaction(
            token.setApprovalForAll(dev, operator.getAddress(), true).join(), 30_000, 500).join();
        assertTrue(token.isApprovedForAll(dev.getAddress(), operator.getAddress()).join());

        client.waitForTransaction(
            token.setApprovalForAll(dev, operator.getAddress(), false).join(), 30_000, 500).join();
        assertFalse(token.isApprovedForAll(dev.getAddress(), operator.getAddress()).join(),
    ...
    @Test @Order(30) @DisplayName("safeTransferFrom() transfers tokens between accounts")
    void safe_transfer_from() throws Exception {
        ...
        // Transfer
        String txHash = token.safeTransferFrom(
            dev, dev.getAddress(), recipient.getAddress(), TOKEN_ID_1, amount, new byte[0]).join();
        client.waitForTransaction(txHash, 30_000, 500).join();
        ...
        assertEquals(devBalBefore.subtract(amount), devBalAfter, "Sender balance must decrease by amount");
    }

    @Test @Order(31) @DisplayName("safeTransferFrom() returns a 0x-prefixed tx hash")
    void safe_transfer_from_returns_hash() throws Exception {
        mint(dev, dev.getAddress(), TOKEN_ID_1, BigInteger.valueOf(5));

        ERC1155 token   = new ERC1155(contractAddr, client);
        String  txHash  = token.safeTransferFrom(
            dev, dev.getAddress(), Wallet.create().getAddress(), TOKEN_ID_1, BigInteger.ONE, new byte[0]).join();
    ...
    void mint(Wallet minter, String to, BigInteger tokenId, BigInteger amount) throws Exception {
        ...
        BigInteger gas     = client.estimateGas(EthModels.CallRequest.builder()
                .from(minter.getAddress()).to(contractAddr).value(BigInteger.ZERO)
                .data(mintData).build()).join()
                .multiply(BigInteger.valueOf(130)).divide(BigInteger.valueOf(100));
        var tx = EthModels.TransactionRequest.builder()
                .from(minter.getAddress()).to(contractAddr).value(BigInteger.ZERO).gas(gas)
                .maxFeePerGas(maxFee).maxPriorityFeePerGas(tip)
                .nonce(nonce).chainId(chainId).data(mintData).build();
        String txHash = client.sendRawTransaction(TransactionSigner.signEip1559(tx, minter)).join();
        client.waitForTransaction(txHash, 30_000, 500).join();
    }

    /**
     * Build calldata for mint(address,uint256,uint256).
     * Selector: keccak256("mint(address,uint256,uint256)")[0:4]
     */
    static String buildMintCalldata(String to, BigInteger tokenId, BigInteger amount) {
        ...
        io.jeth.abi.AbiType[] types = {
            io.jeth.abi.AbiType.ADDRESS,
            io.jeth.abi.AbiType.UINT256,
            io.jeth.abi.AbiType.UINT256
        };
        ...
        byte[] sig      = io.jeth.util.Keccak.hash("mint(address,uint256,uint256)".getBytes(java.nio.charset.StandardCharsets.UTF_8));
