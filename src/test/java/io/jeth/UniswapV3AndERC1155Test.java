...
class UniswapV3AndERC1155Test {
    ...
    static final String WALLET_KEY = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
    ...
    class UniswapV3Tests {
        ...
        @Test @DisplayName("getPoolAddress returns 0x-prefixed address from contract call")
        ...
        @Test @DisplayName("getPoolState throws when pool is zero address")
        ...
        @Test @DisplayName("quoteExactInputSingle returns BigInteger quote")
        void quote_exact_input_single() throws Exception {
            try (var rpc = new RpcMock()) {
                BigInteger expectedQuote = new BigInteger("1800000000000000000"); // ~1800 USDC per ETH
                rpc.enqueue(encodeUint256(expectedQuote));

                var uni = new UniswapV3(rpc.client());
                BigInteger quote = uni.quoteExactInputSingle(
                    TOKEN_A, TOKEN_B, 3000,
        ...
        @Test @DisplayName("quoteExactInputSingle result is positive")
        void quote_positive() throws Exception {
            try (var rpc = new RpcMock()) {
                rpc.enqueue(encodeUint256(BigInteger.valueOf(500_000)));
                var uni = new UniswapV3(rpc.client());
                BigInteger quote = uni.quoteExactInputSingle(
                    TOKEN_A, TOKEN_B, 500, BigInteger.valueOf(1_000_000)).join();
                assertTrue(quote.signum() > 0);
            }
        }

        @Test @DisplayName("batchQuote returns list with same size as input")
        void batch_quote_size() throws Exception {
            try (var rpc = new RpcMock()) {
                ...
                var params = java.util.List.of(
                    new UniswapV3.QuoteParams(TOKEN_A, TOKEN_B, 3000, BigInteger.valueOf(1_000_000)),
                    new UniswapV3.QuoteParams(TOKEN_A, TOKEN_B, 500,  BigInteger.valueOf(1_000_000))
                );
    ...
    class ERC1155Tests {
        ...
        @Test @DisplayName("balanceOf returns BigInteger from eth_call")
        ...
        @Test @DisplayName("balanceOf returns zero correctly")
        ...
        @Test @DisplayName("isApprovedForAll returns true")
        ...
        @Test @DisplayName("isApprovedForAll returns false")
        ...
        @Test @DisplayName("uri returns token metadata URI string")
        ...
        @Test @DisplayName("getAddress returns the contract address")
        ...
        @Test @DisplayName("safeTransferFrom sends a transaction and returns tx hash")
        void safe_transfer_from() throws Exception {
            ...
            try (var rpc = new RpcMock()) {
                ...
                String txHash = token.safeTransferFrom(
                    wallet, ACCOUNT, "0x70997970C51812dc3A010C7d01b50e0d17dc79C8",
                    BigInteger.ONE, BigInteger.valueOf(10), new byte[0]
                ).join();
