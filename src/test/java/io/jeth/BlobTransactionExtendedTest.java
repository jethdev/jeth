...
/**
 * Tests for EIP-4844 BlobTransaction — network format, versioned hashes, signing.
 * Extended from the original BlobTransactionTest to cover the protocol-critical details.
 */
class BlobTransactionExtendedTest {
    ...
    @Test @DisplayName("Blob.versionedHash uses SHA-256 not Keccak")
    void versioned_hash_uses_sha256() throws Exception {
        ...
        for (int i = 1; i < 32; i++) {
            assertEquals(sha256OfCommitment[i], vh[i],
                    "Versioned hash byte[" + i + "] must come from SHA-256(commitment)");
        }
    }

    @Test @DisplayName("BlobTransaction.getBlobVersionedHashes() uses SHA-256 (matches Blob.versionedHash)")
    void blob_tx_versioned_hash_matches_blob() {
        ...
        BlobTransaction tx = BlobTransaction.builder()
                .chainId(1L).nonce(0L)
    ...
    void sign_network_format_has_four_top_level_elements() {
        ...
        assertTrue((rlp[0] & 0xFF) >= 0xC0,
                "After type prefix, signed blob tx must be an RLP list");
    }

    @Test @DisplayName("two blobs → two versioned hashes in tx")
    void two_blobs_two_hashes() {
        ...
        assertFalse(Arrays.equals(tx.getBlobVersionedHashes().get(0),
                                  tx.getBlobVersionedHashes().get(1)),
                "Different blobs must produce different versioned hashes");
    }

    @Test @DisplayName("MAX_BLOBS_PER_TX enforced — 7th blob throws")
    void max_blobs_enforced() {
        BlobTransaction.Builder b = BlobTransaction.builder()
                .chainId(1L).nonce(0L)
                .to("0x70997970C51812dc3A010C7d01b50e0d17dc79C8")
                .gas(BigInteger.valueOf(21000))
                .maxFeePerGas(BigInteger.valueOf(30_000_000_000L))
        ...
        assertThrows(Exception.class, () -> b.blobRaw(new byte[BlobTransaction.BLOB_SIZE]),
                "Should throw when adding blob beyond MAX_BLOBS_PER_TX");
    }

    // ─── padBlob ──────────────────────────────────────────────────────────────

    @Test @DisplayName("padBlob truncates oversized input")
