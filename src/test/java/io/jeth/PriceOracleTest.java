...
/**
 * PriceOracle tests — covers Chainlink round decoding, staleness, batch reads, sqrtPriceX96→price
 * math, tick→price math, and decimal adjustments.
 *
 * <p>All on-chain calls are mocked via RpcMock.
 */
class PriceOracleTest {
    ...
    @Test
    @DisplayName("chainlink() parses 8-decimal feed correctly (ETH/USD = $3412.58)")
    ...
    @Test
    @DisplayName("chainlink() with 6-decimal feed returns correct price")
    void chainlink_6_decimal() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            assertTrue(
                    price.compareTo(new BigDecimal("1.0001")) == 0
                            || price.toPlainString().startsWith("1.0001"));
        }
    }

    @Test
    @DisplayName("chainlink() returns positive price for all known feed constants")
    void chainlink_constants_are_valid_addresses() {
        ...
        for (String feed :
                List.of(
                        PriceOracle.ETH_USD,
                        PriceOracle.BTC_USD,
                        PriceOracle.USDC_USD,
                        PriceOracle.USDT_USD,
                        PriceOracle.DAI_USD,
                        PriceOracle.LINK_USD,
                        PriceOracle.WBTC_USD)) {
    ...
    @Test
    @DisplayName("chainlink() makes exactly 2 RPC calls (latestRoundData + decimals)")
    ...
    @Test
    @DisplayName("isStale() returns false when feed was updated recently")
    ...
    @Test
    @DisplayName("isStale() returns true when feed was updated over maxAge ago")
    ...
    @Test
    @DisplayName("isStale() with very short maxAge always returns true")
    ...
    @Test
    @DisplayName("batchChainlink() with empty list returns empty map without RPC calls")
    ...
    @Test
    @DisplayName("batchChainlink() with 3 feeds enqueues all feeds into Multicall3 (1 RPC call)")
    void batch_chainlink_single_rpc_call() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            oracle.batchChainlink(
                            List.of(PriceOracle.ETH_USD, PriceOracle.BTC_USD, PriceOracle.LINK_USD))
                    .join();
    ...
    @Test
    @DisplayName("batchChainlink() returns map with correct feed addresses as keys")
    ...
    @Test
    @DisplayName("sqrtPriceX96ToPrice: WETH as token0, USDC as token1 (6 decimals)")
    void sqrt_price_weth_usdc() {
        ...
        BigDecimal price =
                PriceOracle.sqrtPriceX96ToPrice(
                        Q96, PriceOracle.WETH, PriceOracle.WETH, PriceOracle.WETH.toLowerCase());
        // When base=quote=token0, price should be 1.0 (same token)
        assertEquals(0, price.compareTo(new BigDecimal("1.000000")));
    }

    @Test
    @DisplayName("sqrtPriceX96ToPrice: inversion when base is token1")
    void sqrt_price_inversion() {
        BigInteger Q96 = BigInteger.TWO.pow(96);
        // When base is token1 and sqrtPriceX96 = Q96 → raw = 1 → invert → 1
        BigDecimal price =
                PriceOracle.sqrtPriceX96ToPrice(
                        Q96, "0xTokenB", "0xTokenA", "0xTokenA"); // base is token1
        assertEquals(0, price.compareTo(new BigDecimal("1.000000")));
    }

    @Test
    @DisplayName("sqrtPriceX96ToPrice: higher sqrtPrice → higher price (monotone)")
    ...
    @Test
    @DisplayName("tickToPrice: WETH/USDC tick=204222 ≈ $3000")
    void tick_eth_usdc_approx() {
        // tick≈204222 corresponds to ~$3000 ETH/USDC (empirically known)
        BigDecimal price =
                PriceOracle.tickToPrice(
                        204222.0,
                        PriceOracle.WETH,
                        PriceOracle.USDC,
                        PriceOracle.WETH.toLowerCase());
    ...
    @Test
    @DisplayName("knownDecimals: USDC and USDT return 6")
    ...
    @Test
    @DisplayName("knownDecimals: WBTC returns 8")
    void known_decimals_8() {
        assertEquals(8, PriceOracle.knownDecimals(PriceOracle.WBTC));
    }

    @Test
    @DisplayName("knownDecimals: WETH, LINK return 18")
    ...
    @Test
    @DisplayName("knownDecimals: unknown address returns 18 (safe default)")
    ...
    @Test
    @DisplayName("ChainlinkRound.ageSeconds() is positive")
    void chainlink_round_age() {
        long updatedAt = System.currentTimeMillis() / 1000 - 500;
        var round =
                new PriceOracle.ChainlinkRound(
                        BigInteger.ONE,
                        new BigDecimal("3412.58"),
                        0L,
                        updatedAt,
                        8,
                        PriceOracle.ETH_USD);
    ...
    @Test
    @DisplayName("ChainlinkRound.isStale() returns correct result")
    ...
    @Test
    @DisplayName("ChainlinkRound.toString() is human-readable")
    ...
    @Test
    @DisplayName("getPool() throws PriceOracleException when pool is zero address")
    void get_pool_throws_for_zero_address() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            assertThrows(
                    Exception.class,
                    () -> oracle.getPool(PriceOracle.WETH, "0xFakeToken", 3000).join());
        }
    }
    ...
    @Test
    @DisplayName("All known token addresses are valid 42-char hex")
