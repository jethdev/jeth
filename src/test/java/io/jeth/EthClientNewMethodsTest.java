...
/**
 * Tests for new EthClient methods: sendTransaction, getAccounts, getNetwork, getLogs auto-chunking,
...
class EthClientNewMethodsTest {
    ...
    @Test
    @DisplayName("MAX_LOG_CHUNK is 2000 (reasonable default)")
    ...
    @Test
    @DisplayName("getLogs with range <= MAX_LOG_CHUNK makes ONE RPC call")
    ...
    @Test
    @DisplayName("getLogs with range > MAX_LOG_CHUNK makes multiple RPC calls (chunking)")
    void get_logs_chunks_large_range() throws Exception {
        ...
        try (var rpc = new RpcMock()) {
            ...
            rpc.enqueueJson(
                    "[{\"address\":\"0xA\",\"topics\":[],\"data\":\"0x\",\"blockNumber\":\"0x1\","
    ...
    @Test
    @DisplayName("getLogs with range exactly MAX_LOG_CHUNK makes 1 call")
    void get_logs_exactly_chunk_size() throws Exception {
        ...
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("[]");
            var client = rpc.client();
            client.getLogs("0x" + Long.toHexString(from), "0x" + Long.toHexString(to), null, null)
                    .join();
    ...
    @Test
    @DisplayName("getLogs with 'latest' toBlock does NOT chunk (string block tag)")
    ...
    @Test
    @DisplayName("getLogsRaw bypasses chunking for large ranges")
    ...
    @Test
    @DisplayName("getLogs merges results from all chunks in order")
    void get_logs_merge_order() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            rpc.enqueueJson(
                    "[{\"address\":\"0xB\",\"topics\":[],\"data\":\"0x\",\"blockNumber\":\"0x7D1\","
    ...
    @Test
    @DisplayName("getAccounts() parses an array of addresses")
    ...
    @Test
    @DisplayName("getAccounts() returns empty list for production nodes")
    ...
    @Test
    @DisplayName("getAccounts() sends eth_accounts method")
    ...
    @Test
    @DisplayName("getNetwork() returns NetworkInfo with correct chainId")
    ...
    @Test
    @DisplayName("getNetwork() resolves known chain name for mainnet")
    ...
    @Test
    @DisplayName("getNetwork() returns 'unknown' for unregistered chain")
    ...
    @Test
    @DisplayName("NetworkInfo.toString() is human-readable")
    ...
    @Test
    @DisplayName("sendTransaction() sends eth_sendTransaction and returns tx hash")
    ...
    @Test
    @DisplayName("sendTransaction() sends eth_sendTransaction method name")
    void send_transaction_method() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueStr("0xabc");
            var req = EthModels.CallRequest.builder().from("0xSender").to("0xTo").build();
