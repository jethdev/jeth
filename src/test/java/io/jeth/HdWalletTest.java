...
/**
 * HD Wallet BIP-32/39/44 tests.
 * Expected addresses verified against MetaMask / ethers.js.
 */
class HdWalletTest {
    ...
    @Test @DisplayName("Known mnemonic derives correct addresses (0-4)")
    void known_mnemonic() {
    ...
    @Test @DisplayName("Derivation is deterministic")
    void deterministic() {
        assertEquals(HdWallet.fromMnemonic(MNEMONIC).getAccount(0).getAddress(),
                     HdWallet.fromMnemonic(MNEMONIC).getAccount(0).getAddress());
    }

    @Test @DisplayName("getMnemonic() round-trips")
    void mnemonic_preserved() {
        assertEquals(MNEMONIC, HdWallet.fromMnemonic(MNEMONIC).getMnemonic());
    }

    @Test @DisplayName("generate() produces valid 12-word mnemonic")
    void generate_12() {
    ...
    @Test @DisplayName("generate(256) produces 24-word mnemonic")
    void generate_24() {
        assertEquals(24, HdWallet.generate(256).getMnemonic().split("\\s+").length);
    }

    @Test @DisplayName("Each generate() produces unique mnemonic")
    void generate_unique() {
        assertNotEquals(HdWallet.generate().getMnemonic(), HdWallet.generate().getMnemonic());
    }

    @Test @DisplayName("Generated wallet restores from its own mnemonic")
    ...
    @Test @DisplayName("getAccounts(5) returns 5 distinct valid addresses")
    void get_accounts() {
    ...
    @Test @DisplayName("derive(path) == getAccount(0) for standard path")
    void derive_standard_path() {
        var hd = HdWallet.fromMnemonic(MNEMONIC);
        assertEquals(hd.getAccount(0).getAddress(), hd.derive("m/44'/60'/0'/0/0").getAddress());
    }

    @Test @DisplayName("Passphrase changes derived addresses")
    void passphrase() {
    ...
    @Test @DisplayName("fromSeed round-trips")
    void from_seed() {
        var hd = HdWallet.fromMnemonic(MNEMONIC);
        assertEquals(hd.getAccount(0).getAddress(), HdWallet.fromSeed(hd.getSeed()).getAccount(0).getAddress());
    }
}
