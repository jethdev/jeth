...
/**
 * Extended Wallet tests: signing, recovery, personal_sign.
 * Core creation tests live in CryptoTest.
 */
class WalletExtendedTest {

    static final Wallet HARDHAT_0 = Wallet.fromPrivateKey("0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80");
    static final Wallet HARDHAT_1 = Wallet.fromPrivateKey("0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d");

    // ── Recovery ─────────────────────────────────────────────────────────────

    @Test @DisplayName("recoverAddress from hash + sig returns original signer")
    ...
    @Test @DisplayName("recoverAddress: wrong hash → wrong signer")
    void recover_wrong_hash() {
        byte[] hash1 = Hex.decode("0x1111111111111111111111111111111111111111111111111111111111111111");
        byte[] hash2 = Hex.decode("0x2222222222222222222222222222222222222222222222222222222222222222");
    ...
    @Test @DisplayName("personal_sign: recoverPersonalMessage returns signer")
    ...
    @Test @DisplayName("personal_sign: different wallets produce different sigs")
    ...
    @Test @DisplayName("hashPersonalMessage adds Ethereum prefix")
    ...
    @Test @DisplayName("All sigs have low-S (EIP-2 canonical form)")
    ...
    @Test @DisplayName("Signature.toBytes() = 65 bytes [r(32) + s(32) + v(1)]")
    ...
    @Test @DisplayName("Signature.toHex() is 132-char hex string (0x + 130 chars)")
    void sig_hex_length() {
        byte[] hash = Hex.decode("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
        Signature sig = HARDHAT_0.sign(hash);
    ...
    @Test @DisplayName("Sign: v is 0 or 1 (pre-EIP-155 form)")
    void sig_v_values() {
        for (int i = 0; i < 5; i++) {
    ...
    @Test @DisplayName("signMessage: v is 27 or 28 (personal_sign form)")
    ...
    @Test @DisplayName("All 10 Hardhat dev accounts derive correctly")
