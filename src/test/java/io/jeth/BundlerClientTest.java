 ...
 *
 * <p>Regression: BundlerClient.poll() previously compared void == null (compile error).
 * Fixed to use supplyAsync + delayedExecutor. These tests verify the polling API works.
 */
class BundlerClientTest {
    ...
    @Test @DisplayName("sendUserOperation returns userOpHash string from bundler")
    ...
    @Test @DisplayName("sendUserOperation makes exactly one RPC call")
    ...
    @Test @DisplayName("sendUserOperation throws on bundler RPC error")
    ...
    @Test @DisplayName("estimateUserOperationGas returns GasEstimate with positive fields")
    void estimate_gas_returns_estimate() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"callGasLimit\":\"0x186a0\",\"verificationGasLimit\":\"0x249f0\",\"preVerificationGas\":\"0x5208\"}");
            var client = BundlerClient.of(rpc.url());
    ...
    @Test @DisplayName("getSupportedEntryPoints returns list of addresses")
    ...
    @Test @DisplayName("getSupportedEntryPoints returns empty list when bundler supports none")
    ...
    @Test @DisplayName("getUserOperationReceipt returns null when not yet mined")
    ...
    @Test @DisplayName("waitForUserOperationReceipt returns immediately when already mined")
    @Timeout(5)
    ...
    @Test @DisplayName("waitForUserOperationReceipt times out when op never mines")
    @Timeout(5)
    void wait_for_receipt_timeout() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            assertThrows(Exception.class,
                () -> client.waitForUserOperationReceipt("0x" + "ab".repeat(32), 200, 50).join(),
                "Should throw timeout exception when op never mines");
        }
    }

    @Test @DisplayName("waitForUserOperationReceipt eventually resolves after pending polls")
    @Timeout(5)
    void wait_for_receipt_after_pending() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            var client = BundlerClient.of(rpc.url());
            var receipt = client.waitForUserOperationReceipt(
                "0x" + "ab".repeat(32), 10_000, 50).join();
    ...
    @Test @DisplayName("BundlerClient.of(url) creates a client")
