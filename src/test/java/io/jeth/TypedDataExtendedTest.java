...
/**
 * Extended EIP-712 tests covering hashTypedData, hashStruct, encodeData, and
 * Domain.separator() — the building blocks that sign() delegates to.
 *
 * <p>Known vectors from the EIP-712 spec and MetaMask reference implementations
 * are used where possible to ensure spec compliance.
 */
class TypedDataExtendedTest {
    ...
    static final TypedData.Domain DOMAIN = TypedData.Domain.builder()
            .name("TestApp").version("1").chainId(1L)
    ...
    static final Map<String, Object> MESSAGE = Map.of(
    ...
    @Test @DisplayName("separator() returns 32 bytes")
    void separator_length() {
        byte[] sep = DOMAIN.separator();
        assertEquals(32, sep.length, "domain separator must be 32 bytes");
    }

    @Test @DisplayName("separator() is deterministic")
    void separator_deterministic() {
        assertArrayEquals(DOMAIN.separator(), DOMAIN.separator());
    }

    @Test @DisplayName("separator() differs when chainId changes")
    void separator_chainid_unique() {
        TypedData.Domain d1 = TypedData.Domain.builder().name("App").version("1").chainId(1L).build();
        TypedData.Domain d2 = TypedData.Domain.builder().name("App").version("1").chainId(137L).build();
        assertFalse(java.util.Arrays.equals(d1.separator(), d2.separator()),
                "Different chainIds must produce different separators");
    }

    @Test @DisplayName("separator() differs when name changes")
    void separator_name_unique() {
        TypedData.Domain d1 = TypedData.Domain.builder().name("App1").version("1").chainId(1L).build();
        TypedData.Domain d2 = TypedData.Domain.builder().name("App2").version("1").chainId(1L).build();
        assertFalse(java.util.Arrays.equals(d1.separator(), d2.separator()));
    }

    @Test @DisplayName("separator() differs when version changes")
    void separator_version_unique() {
        TypedData.Domain d1 = TypedData.Domain.builder().name("App").version("1").chainId(1L).build();
        TypedData.Domain d2 = TypedData.Domain.builder().name("App").version("2").chainId(1L).build();
        assertFalse(java.util.Arrays.equals(d1.separator(), d2.separator()));
    }

    @Test @DisplayName("separator() differs when verifyingContract changes")
    void separator_contract_unique() {
        TypedData.Domain d1 = TypedData.Domain.builder().name("A").version("1").chainId(1L)
                .verifyingContract("0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC").build();
        TypedData.Domain d2 = TypedData.Domain.builder().name("A").version("1").chainId(1L)
                .verifyingContract("0xDDdDddDdDdddDDddDDddDDDDdDdDDdDDdDDDDDd").build();
        assertFalse(java.util.Arrays.equals(d1.separator(), d2.separator()));
    }

    // ─── hashStruct() ─────────────────────────────────────────────────────────

    @Test @DisplayName("hashStruct() returns 32 bytes")
    void hash_struct_length() {
        byte[] h = TypedData.hashStruct("Mail", TYPES, MESSAGE);
        assertEquals(32, h.length);
    }

    @Test @DisplayName("hashStruct() is deterministic")
    ...
    @Test @DisplayName("hashStruct() changes when message field changes")
    void hash_struct_message_sensitive() {
        Map<String, Object> msg2 = Map.of(
                "from",     "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266",
                "to",       "0x70997970C51812dc3A010C7d01b50e0d17dc79C8",
                "contents", "Different!"
        );
    ...
    @Test @DisplayName("hashStruct() changes when type definition changes")
    void hash_struct_type_sensitive() {
        Map<String, List<TypedData.Field>> types2 = Map.of(
                "Mail", List.of(
                        TypedData.Field.of("from",    "address"),
                        TypedData.Field.of("to",      "address"),
                        TypedData.Field.of("contents","string"),
    ...
    @Test @DisplayName("hashTypedData() returns 32 bytes")
    void hash_typed_data_length() {
        byte[] h = TypedData.hashTypedData(DOMAIN, "Mail", TYPES, MESSAGE);
        assertEquals(32, h.length);
    }

    @Test @DisplayName("hashTypedData() is deterministic")
    ...
    @Test @DisplayName("hashTypedData() starts with 0x1901 prefix (EIP-712 magic bytes)")
    ...
    @Test @DisplayName("hashTypedData() changes when domain changes")
    void hash_typed_data_domain_sensitive() {
        TypedData.Domain d2 = TypedData.Domain.builder()
                .name("OtherApp").version("1").chainId(1L).build();
        assertFalse(java.util.Arrays.equals(
                TypedData.hashTypedData(DOMAIN,  "Mail", TYPES, MESSAGE),
    ...
    @Test @DisplayName("hashTypedData() changes when message changes")
    ...
    @Test @DisplayName("hashTypedData() matches: hash = keccak(separator || structHash)")
    void hash_typed_data_decomposition() {
        ...
        preImage[0] = 0x19; preImage[1] = 0x01;
        ...
        assertArrayEquals(expected, full, "hashTypedData must equal keccak256(0x1901||sep||struct)");
    }

    // ─── encodeData() ─────────────────────────────────────────────────────────

    @Test @DisplayName("encodeData() length = 32 * (1 + numFields) for static struct")
    ...
    @Test @DisplayName("encodeData() is deterministic")
    ...
    @Test @DisplayName("encodeData() first 32 bytes = typeHash")
    ...
    @Test @DisplayName("encodeData() changes when a field value changes")
    ...
    @Test @DisplayName("struct with uint256 field: hashStruct consistent with sign()")
    ...
    @Test @DisplayName("sign() signs exactly hashTypedData() output")
    void sign_signs_hash_typed_data() {
        io.jeth.crypto.Wallet wallet = io.jeth.crypto.Wallet.fromPrivateKey(
