...
class TxTracerTest {
    ...
    @Test @DisplayName("trace() parses simple CALL trace")
    ...
    @Test @DisplayName("trace() captures revert error")
    ...
    @Test @DisplayName("nested calls: depth() and callCount()")
    ...
    @Test @DisplayName("render() produces non-empty string")
    ...
    @Test @DisplayName("render() includes from → to")
    void render_contains_addresses() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson(TRACE_JSON);
            String rendered = TxTracer.of(rpc.client()).trace("0xtx").join().render();
            assertTrue(rendered.contains("0xfrom") || rendered.contains("0xto"),
                "render should contain addresses. Got: " + rendered);
        }
    }

    @Test @DisplayName("CallTrace.empty() is successful with 0 depth")
