...
/**
 * Unit tests for EthClient advanced methods not covered by EthClientTest: getStorageAt,
 * getBlobBaseFee, traceTransaction, traceCall, createAccessList, callWithOverride, and getProvider.
 */
class EthClientAdvancedTest {
    ...
    @Test
    @DisplayName("getStorageAt sends eth_getStorageAt with correct params")
    void get_storage_at_method() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            String result =
                    rpc.client()
                            .getStorageAt(
                                    "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", BigInteger.ZERO)
                            .join();
    ...
    @Test
    @DisplayName("getStorageAt returns 0x-prefixed hex string")
    ...
    @Test
    @DisplayName("getStorageAt with explicit block tag sends 3-param call")
    ...
    @Test
    @DisplayName("getStorageAt default block = 'latest'")
    ...
    @Test
    @DisplayName("getBlobBaseFee sends eth_blobBaseFee")
    ...
    @Test
    @DisplayName("getBlobBaseFee returns BigInteger")
    ...
    @Test
    @DisplayName("traceTransaction sends debug_traceTransaction")
    ...
    @Test
    @DisplayName("traceTransaction returns JsonNode")
    ...
    @Test
    @DisplayName("traceTransaction with opts includes opts in request")
    void trace_transaction_with_opts() throws Exception {
        try (var rpc = new RpcMock()) {
            rpc.enqueueJson("{\"structLogs\":[]}");
            rpc.client()
                    .traceTransaction("0x" + "c".repeat(64), Map.of("tracer", "callTracer"))
                    .join();
            String body = rpc.takeRequest().getBody().readUtf8();
    ...
    @Test
    @DisplayName("traceCall sends debug_traceCall")
    ...
    @Test
    @DisplayName("traceCall returns JsonNode result")
    ...
    @Test
    @DisplayName("createAccessList sends eth_createAccessList")
    ...
    @Test
    @DisplayName("createAccessList returns JsonNode with accessList field")
    void create_access_list_returns_json() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            assertNotNull(result);
            assertTrue(
                    result.has("accessList") || result.has("gasUsed"),
    ...
    @Test
    @DisplayName("callWithOverride sends eth_call with state override")
    ...
    @Test
    @DisplayName("callWithOverride returns hex result string")
    ...
    @Test
    @DisplayName("getProvider returns the HttpProvider backing the client")
    void get_provider_is_http() throws Exception {
        try (var rpc = new RpcMock()) {
            ...
            assertTrue(
                    provider instanceof io.jeth.provider.HttpProvider
