...
class MiddlewareProviderTest {
    ...
    @Test @DisplayName("Cache hit: repeated cacheable call hits provider once")
    void cache_hit() {
        var calls = new AtomicInteger();
        var provider = MiddlewareProvider.wrap(stub(calls, "\"0x1\""))
            .withCache(Duration.ofSeconds(60)).build();
    ...
    @Test @DisplayName("Cache miss: eth_sendRawTransaction always hits provider")
    void cache_miss_non_cacheable() {
        var calls = new AtomicInteger();
        var provider = MiddlewareProvider.wrap(stub(calls, "\"0xtxhash\""))
            .withCache(Duration.ofSeconds(60)).build();
    ...
    @Test @DisplayName("clearCache() forces fresh fetch on next call")
    void clear_cache() {
        var calls = new AtomicInteger();
        var provider = MiddlewareProvider.wrap(stub(calls, "\"0x1\""))
            .withCache(Duration.ofSeconds(60)).build();
    ...
    @Test @DisplayName("totalRequests increments with each call")
    void metrics_total() {
    ...
    @Test @DisplayName("cacheHitRate() is 75% for 1 miss + 3 hits")
    void metrics_hit_rate() {
        var provider = MiddlewareProvider.wrap(stub(new AtomicInteger(), "\"0x1\""))
            .withCache(Duration.ofSeconds(60)).build();
        var req = req("eth_chainId");
        provider.send(req).join(); // miss
        provider.send(req).join(); // hit
    ...
    @Test @DisplayName("getMetrics().toString() has all fields")
    ...
    @Test @DisplayName("withFallback: uses first healthy provider")
    ...
    @Test @DisplayName("withFallback: falls back when first provider throws")
    ...
    @Test @DisplayName("withFallback: throws when ALL providers fail")
        ...
        return new Provider() {
            public CompletableFuture<RpcModels.RpcResponse> send(RpcModels.RpcRequest req) {
                ...
                try {
                    ...
                    r.id = req.id; r.result = MAPPER.readTree(resultJson);
