...
/**
 * Represents a Solidity contract function — computes selector, encodes calldata, decodes return
 * values.
 */
public class Function {
    ...
    public Object[] decodeReturn(String hexResult) {
        if (outputTypes.length == 0)
            throw new AbiException("No output types declared — use .withReturns(...)");
    ...
    public String selector() {
        return getSelectorHex();
    }

    public byte[] getSelector() {
        return Arrays.copyOf(selector, 4);
    }
    ...
    public AbiType[] getOutputTypes() {
        return Arrays.copyOf(outputTypes, outputTypes.length);
    }
