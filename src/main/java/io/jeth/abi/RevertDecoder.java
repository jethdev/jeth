...
/**
 * Decode Solidity custom errors into human-readable names — with a built-in
 * registry of known errors from every major protocol.
 *
 * Unlike {@link AbiDecodeError} which only decodes {@code Error(string)} and
 * {@code Panic(uint256)}, this class resolves the 4-byte selector of ANY custom
 * error to its name if the protocol is known.
 *
 * <pre>
 ...
 * Built-in registries:
 * <ul>
 *   <li>OpenZeppelin — ERC-20, ERC-721, ERC-1155, AccessControl, Ownable, Pausable, ReentrancyGuard</li>
 *   <li>Uniswap V3 — Router, Pool, Factory</li>
 *   <li>Aave V3 — Pool errors (all 80+)</li>
...
public final class RevertDecoder {
     ...
     * <ol>
     *   <li>{@code Error(string)} — {@code "execution reverted: transfer amount exceeds balance"}</li>
     *   <li>{@code Panic(uint256)} — {@code "execution reverted (Panic 0x11): arithmetic overflow"}</li>
     *   <li>Known custom error — {@code "OZ/ERC-20: ERC20InsufficientBalance"} or
     *       {@code "Aave/V3: HEALTH_FACTOR_NOT_BELOW_THRESHOLD"}</li>
     *   <li>Unknown custom error — {@code "custom error 0x1234abcd"}</li>
     *   <li>Empty/null — {@code "execution reverted"}</li>
     * </ol>
     *
    ...
    /**
     * Look up only the name of a custom error by its 4-byte selector.
     * Returns {@code Optional.empty()} if not in the registry.
     *
     * @param selectorHex "0x1425ea42" or "1425ea42"
     */
    public static Optional<String> lookup(String selectorHex) {
        String key = selectorHex.startsWith("0x") ? selectorHex.substring(2).toLowerCase()
    ...
    public static void registerError(String errorSignature, String protocol) {
        ...
        String selector = String.format("%02x%02x%02x%02x",
    ...
    /**
     * Aave V3 encodes errors as revert with a numeric string code.
     * We compute the 4-byte selector of the custom error as if it were:
     * {@code error AaveError(uint256 code)} with the given numeric value.
     * This is a best-effort approximation — actual Aave V3 uses string reverts.
     */
