 ...
 * <ul>
 *   <li>Integers: {@code uint8} to {@code uint256}, {@code int8} to {@code int256}</li>
 *   <li>Fixed bytes: {@code bytes1} to {@code bytes32}</li>
 *   <li>Primitive: {@code address}, {@code bool}, dynamic {@code bytes}, {@code string}</li>
 *   <li>Fixed arrays: {@code T[N]} via {@link #arrayOf(AbiType, int)}</li>
 *   <li>Dynamic arrays: {@code T[]} via {@link #arrayOf(AbiType)}</li>
 *   <li>Tuples (structs): {@code (T1, T2, ...)} via {@link #tuple(AbiType...)}</li>
 *   <li>Nested: {@code (address, uint256)[]} via {@link #of(String)}</li>
 * </ul>
 *
 * <pre>
 * AbiType.UINT256
 ...
 * </pre>
 *
 * <p>Instances are reusable and thread-safe. Store them as static constants
 * (as done in {@link io.jeth.price.PriceOracle}) to avoid re-parsing signatures on every call.
 *
...
public class AbiType {
     ...
     * Handles: uint256, address, bool, string, bytes, bytesN,
     * T[], T[N], (T1,T2,...), (T1,T2,...)[], (T1,T2,...)[N]
     */
        ...
        return switch (typeName) {
            ...
            default -> {
                ...
                if (typeName.startsWith("bytes") && typeName.length() > 5) {
                    ...
                    if (sz < 1 || sz > 32) throw new AbiException("Invalid bytesN size: " + typeName);
    ...
    public boolean isTuple()               { return tupleTypes != null; }
    public boolean isDynamicArray()        { return isArray() && arraySize == -1; }
    public boolean isFixedArray()          { return isArray() && arraySize >= 0; }
    public AbiType getArrayElementType()   { return arrayElementType; }
    public int     getArraySize()          { return arraySize; }
    public AbiType[] getTupleTypes()       { return tupleTypes != null ? Arrays.copyOf(tupleTypes, tupleTypes.length) : null; }

    @Override
    public String toString() {
        if (isTuple()) {
            String inner = Arrays.stream(tupleTypes).map(AbiType::toString).collect(Collectors.joining(","));
            return "(" + inner + ")";
        }
        if (isArray()) {
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(baseType, t.baseType) && size == t.size && dynamic == t.dynamic
            && Objects.equals(arrayElementType, t.arrayElementType) && arraySize == t.arraySize
            && Arrays.equals(tupleTypes, t.tupleTypes);
    }

    @Override public int hashCode() {
        return Objects.hash(baseType, size, dynamic, arrayElementType, arraySize, Arrays.hashCode(tupleTypes));
    }
}
