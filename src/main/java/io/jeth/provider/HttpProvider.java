...
public class HttpProvider implements Provider {
    ...
    private HttpProvider(Builder b) {
        ...
        this.httpClient = b.httpClient != null ? b.httpClient
                : HttpClient.newBuilder()
                    ...
                    .executor(b.executor != null ? b.executor : ForkJoinPool.commonPool())
                    .build();
    ...
    public CompletableFuture<RpcModels.RpcResponse> send(RpcModels.RpcRequest request) {
        byte[] body;
        try { body = mapper.writeValueAsBytes(request); }
        catch (Exception e) { return CompletableFuture.failedFuture(
        ...
        return httpClient.sendAsync(reqBuilder.build(), HttpResponse.BodyHandlers.ofByteArray())
                .thenApply(resp -> {
                    int status = resp.statusCode();
                    if (status == 429) throw new EthException("Rate limited (429) by " + rpcUrl.getHost());
                    if (status == 401) throw new EthException("Unauthorized (401) — check API key for " + rpcUrl.getHost());
                    if (status != 200) throw new EthException("HTTP " + status + " from " + rpcUrl.getHost());
                    try { return mapper.readValue(resp.body(), RpcModels.RpcResponse.class); }
                    catch (Exception e) { throw new EthException("Failed to parse RPC response", e); }
                });
    }

    @Override public ObjectMapper getObjectMapper() { return mapper; }
    @Override public void close() {} // HttpClient is not closeable in Java 11-17
    ...
    public static class Builder {
        ...
        Builder(String url) { this.uri = URI.create(url); }
        ...
        public HttpProvider build() { return new HttpProvider(this); }
    }
}
