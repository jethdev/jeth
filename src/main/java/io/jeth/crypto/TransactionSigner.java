...
/**
 * Signs Ethereum transactions. Supports EIP-1559 (type 2), legacy (type 0),
 * and EIP-2930 access-list (type 1) transactions.
 */
public class TransactionSigner {

    /**
     * Sign an EIP-1559 (type 2) transaction.
     * Returns a 0x-prefixed raw transaction hex for eth_sendRawTransaction.
     */
    public static String signEip1559(EthModels.TransactionRequest tx, Wallet wallet) {
        ...
        BigInteger tip  = tx.maxPriorityFeePerGas  != null ? tx.maxPriorityFeePerGas  : BigInteger.ZERO;

        // EIP-1559: 0x02 || RLP([chainId, nonce, maxPriorityFee, maxFee, gas, to, value, data, accessList])
        List<Object> unsigned = List.of(
                BigInteger.valueOf(tx.chainId), BigInteger.valueOf(tx.nonce),
                tip, maxF, gas, to, val, data, List.of());

        byte[] payload = typed(0x02, Rlp.encode(unsigned));
        byte[] hash    = Keccak.hash(payload);
        Signature sig  = wallet.sign(hash);

        List<Object> signed = List.of(
                BigInteger.valueOf(tx.chainId), BigInteger.valueOf(tx.nonce),
                tip, maxF, gas, to, val, data, List.of(),
    ...
    public static String signLegacy(EthModels.TransactionRequest tx, Wallet wallet) {
        ...
        List<Object> unsigned = List.of(
                BigInteger.valueOf(tx.nonce), gp, gas, to, val, data,
                BigInteger.valueOf(tx.chainId), BigInteger.ZERO, BigInteger.ZERO);
        ...
        List<Object> signed = List.of(
                BigInteger.valueOf(tx.nonce), gp, gas, to, val, data, v, sig.r, sig.s);
     ...
     * accessList format: list of (address, storageKeys[]) pairs.
     */
    public static String signEip2930(EthModels.TransactionRequest tx, Wallet wallet,
        ...
        List<Object> encodedAccessList = accessList.stream()
                .map(e -> (Object) List.of(Hex.decode(e.address()),
                        e.storageKeys().stream().map(Hex::decode).toList()))
                .toList();

        List<Object> unsigned = List.of(
                BigInteger.valueOf(tx.chainId), BigInteger.valueOf(tx.nonce),
                gp, gas, to, val, data, encodedAccessList);
    ...
    private static BigInteger rlpBytes(Object item) {
        ...
        throw new IllegalArgumentException("Expected byte[] from RLP decoder, got: " + item.getClass());
    }
    ...
    /**
     * Recover the signer address from a signed raw transaction hex.
     * Supports EIP-1559 (type 0x02) and legacy transactions.
     */
    ...
    public static String recoverSigner(String rawTxHex) {
        ...
        if (raw[0] == 0x02) {
            ...
            List<?> unsigned = List.of(decoded.get(0), decoded.get(1), decoded.get(2),
                    decoded.get(3), decoded.get(4), decoded.get(5),
                    BigInteger.valueOf(chainId), BigInteger.ZERO, BigInteger.ZERO);
