 ...
 *
 * <p>Decodes debug_traceTransaction into a tree of contract calls, showing exactly what happened
...
public class TxTracer {
    ...
    private TxTracer(EthClient client) {
        this.client = client;
    }

    public static TxTracer of(EthClient client) {
        return new TxTracer(client);
    }
    ...
    private static CallTrace parseTrace(JsonNode node) {
        ...
        return new CallTrace(
                type, from, to, selector, value, gas, gasUsed, error, revertReason, output, calls);
    }
    ...
    public record CallTrace(
        ...
        public static CallTrace empty() {
            return new CallTrace(
                    "CALL", "", "", "0x", "0x0", "0x0", "0x0", null, null, "0x", List.of());
        }
        ...
        public int depth() {
            return calls.isEmpty()
                    ? 0
                    : 1 + calls.stream().mapToInt(CallTrace::depth).max().orElse(0);
        }

        public int callCount() {
            return calls.size() + calls.stream().mapToInt(CallTrace::callCount).sum();
        }
        ...
        private void render(StringBuilder sb, int indent) {
            ...
            if (!value.equals("0x0") && !value.equals("0x")) sb.append("  value=").append(value);
