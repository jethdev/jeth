...
public class MiddlewareProvider implements Provider {
    ...
    private static final Set<String> CACHEABLE_METHODS =
            Set.of("eth_chainId", "net_version", "eth_getCode");
                ...
                .whenComplete(
                        (resp, ex) -> {
                            ...
                            if (ex != null) {
                                ...
                                if (loggingEnabled)
                                    log.fine("← " + request.method + " (" + ms + "ms)");
                                if (cache != null
                                        && CACHEABLE_METHODS.contains(request.method)
                                        && resp != null
                                        && !resp.hasError()) {
                                    String key = request.method + ":" + request.params;
                                    responseCache.put(
                                            ...
                                            new CacheEntry(
                                                    resp,
                                                    System.currentTimeMillis() + cache.ttlMs));
                                }
                            }
                        });
    }

    private CompletableFuture<RpcModels.RpcResponse> sendWithRetry(
            RpcModels.RpcRequest req, int attempt) {
        return inner.send(req)
                ...
                .exceptionallyCompose(
                        ex -> {
                            ...
                            CompletableFuture<RpcModels.RpcResponse> failed =
                                    new CompletableFuture<>();
                            failed.completeExceptionally(
                                    ex instanceof RuntimeException re
                                            ? re
                                            : new EthException("RPC failed after retries", ex));
                            return failed;
                        });
    }
    ...
    public static Builder wrap(Provider inner) {
    ...
    public static class Builder {
        ...
        Builder(Provider inner) {
            this.inner = inner;
        }

        public Builder withRetry(int maxAttempts, Duration baseDelay) {
            this.retry = new RetryConfig(maxAttempts, baseDelay.toMillis());
            return this;
        }

        public Builder withRateLimit(int maxPerSecond) {
            this.rateLimit = new RateLimitConfig(maxPerSecond);
            return this;
        }

        public Builder withCache(Duration ttl) {
            this.cache = new CacheConfig(ttl.toMillis());
            return this;
        }
        ...
        public MiddlewareProvider build() {
            return new MiddlewareProvider(this);
        }
    }
    ...
    public static class Metrics {
        ...
        public String toString() {
            return String.format(
                    "Metrics{requests=%d, cacheHits=%d (%.1f%%), retries=%d, errors=%d, avgLatency=%.1fms}",
                    totalRequests.get(),
                    cacheHits.get(),
                    cacheHitRate(),
                    retries.get(),
    ...
    static class FallbackProvider implements Provider {
        ...
        FallbackProvider(List<Provider> p) {
            this.providers = p;
        }
        ...
        private static final Logger fallbackLog =
                Logger.getLogger(FallbackProvider.class.getName());

        private CompletableFuture<RpcModels.RpcResponse> tryNext(RpcModels.RpcRequest req, int i) {
            if (i >= providers.size())
                return CompletableFuture.failedFuture(
                        new EthException("All providers failed: " + req.method));
            return providers
                    .get(i)
                    .send(req)
                    .exceptionally(
        ...
        @Override
        public ObjectMapper getObjectMapper() {
            return providers.get(0).getObjectMapper();
        }
