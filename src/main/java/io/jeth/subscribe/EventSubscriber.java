...
/**
 * @deprecated Use {@link io.jeth.contract.ContractEvents} instead. ContractEvents provides the same
...
public class EventSubscriber {
    ...
    private EventSubscriber(WsProvider ws) {
        this.ws = ws;
    }

    public static EventSubscriber of(WsProvider ws) {
        return new EventSubscriber(ws);
    }
    ...
    public CompletableFuture<String> on(
        ...
        return ws.onLogs(
                ...
                logNode -> {
                    try {
                        EthModels.Log log =
                                new ObjectMapper().treeToValue(logNode, EthModels.Log.class);
    ...
    public WsProvider getWsProvider() {
        return ws;
    }
}
