 ...
 *
 * <p>All amounts use the token's native raw integer representation.
 * To convert between human-readable and raw amounts, use {@link io.jeth.util.Units}:
 *
 * <pre>
...
public class ERC20 {
    ...
    public CompletableFuture<BigInteger> balanceOf(String account)               { return fnBalanceOf.call(account).as(BigInteger.class); }
    public CompletableFuture<BigInteger> totalSupply()                            { return fnTotalSupply.call().as(BigInteger.class); }
    public CompletableFuture<BigInteger> allowance(String owner, String spender) { return fnAllowance.call(owner, spender).as(BigInteger.class); }
    ...
    public CompletableFuture<BigInteger> nonces(String owner)                    { return fnNonces.call(owner).as(BigInteger.class); }
     ...
     *
     * <p>Makes two RPC calls: {@code decimals()} and {@code balanceOf()}, then divides.
     * For batching multiple balances with a single RPC call, use
     * {@link io.jeth.multicall.Multicall3#getTokenBalances}.
     *
     * @param account the address to query
     */
    public CompletableFuture<BigDecimal> balanceOfFormatted(String account) {
        return decimals().thenCompose(dec ->
                balanceOf(account).thenApply(bal -> Units.fromWei(bal, dec)));
    }
    ...
    public CompletableFuture<String> transferFrom(Wallet wallet, String from, String to, BigInteger amount) {
     ...
     *
     * <p>Internally: fetches {@code name()}, {@code nonces(owner)}, and {@code chainId},
     * constructs and signs the EIP-712 Permit digest, then calls
     * {@code permit(owner, spender, value, deadline, v, r, s)} on-chain.
     *
     * <p>This replaces the classic two-transaction {@code approve + transferFrom} pattern
     * with a single transaction, saving ~50k gas and eliminating a user confirmation step.
     * Compatible with USDC, DAI, UNI, AAVE, and any EIP-2612 token.
     *
     ...
     * @param deadline unix timestamp after which the permit is invalid (use a short window, e.g. now + 20 min)
    ...
    public CompletableFuture<String> permit(
        ...
        return name().thenCompose(tokenName ->
            nonces(owner.getAddress()).thenCompose(nonce ->
                contract.getClient().getChainId().thenCompose(chainId -> {
                    Signature sig = TypedData.signPermit(
                            contract.getAddress(), tokenName, "1", chainId,
                            owner.getAddress(), spender, value, nonce, deadline, owner);
                    BigInteger v = BigInteger.valueOf(sig.v); // TypedData.sign already normalizes to 27 or 28
                    byte[] r = toBe32(sig.r);
                    byte[] s = toBe32(sig.s);
                    return fnPermit.send(owner, owner.getAddress(), spender, value, deadline, v, r, s);
                })));
    }

    /** Max uint256 — use for unlimited approval. */
    public static final BigInteger MAX_APPROVAL =
