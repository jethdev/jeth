...
/**
 * Low-level contract wrapper: define functions from human-readable Solidity signatures,
 * then call or send them without needing an ABI JSON file.
 *
 * <p>For a fully typed experience with auto-mapped return types, use
 * {@link ContractProxy} instead. {@code Contract} is useful when you only need
 * one or two functions and don't want to define a Java interface.
 *
 * <pre>
...
public class Contract {
     ...
     * </pre>
     *
     * <p>The returned {@link FunctionBuilder} lets you specify return types
     * before calling or sending.
     *
    ...
    /**
     * Bind a pre-built {@link Function} to this contract.
     * Avoids re-parsing the signature each time — useful for frequently-called ABIs
     * stored as static constants (e.g. in {@link io.jeth.price.PriceOracle}).
     */
        ...
        return client.getChainId().thenCompose(chainId ->
            ...
            client.getMaxPriorityFeePerGas().thenCompose(tip -> {
                ...
                var tx = EthModels.TransactionRequest.builder()
                        .from(wallet.getAddress()).to(to).value(valueWei)
    ...
    public static CompletableFuture<String> deploy(
            ...
            AbiType[] ctorTypes, Object[] ctorArgs) {
        byte[] initcode;
        ...
        return client.getChainId().thenCompose(chainId ->
            ...
            client.getMaxPriorityFeePerGas().thenCompose(tip -> {
                ...
                var estimateReq = EthModels.CallRequest.builder()
                        .from(wallet.getAddress()).data(dataHex).build();
                return client.estimateGas(estimateReq).thenCompose(gas -> {
                    BigInteger gasWithBuffer = gas.multiply(BigInteger.valueOf(130))
                                                  .divide(BigInteger.valueOf(100));
                    var tx = EthModels.TransactionRequest.builder()
                            .from(wallet.getAddress()).value(BigInteger.ZERO)
                            .gas(gasWithBuffer).maxFeePerGas(maxFee).maxPriorityFeePerGas(tip)
                            .nonce(nonce).chainId(chainId).data(dataHex).build();
                    String signed = TransactionSigner.signEip1559(tx, wallet);
                    return client.sendRawTransaction(signed)
                            .thenCompose(client::waitForTransaction)
     ...
     *
     * Auto-chunks large block ranges to avoid RPC "response size exceeded" errors.
     *
     * <pre>
     ...
     * </pre>
     */
    public CompletableFuture<List<EventDef.DecodedEvent>>
            queryFilter(EventDef event, long fromBlock, long toBlock) {
    ...
    public CompletableFuture<List<EventDef.DecodedEvent>>
            queryFilter(EventDef event, long fromBlock) {
        return client.getLogs(
                Hex.fromLong(fromBlock), "latest", address,
                List.of(event.topic0Hex())
        ).thenApply(event::decodeAll);
    }
    ...
    public class FunctionBuilder {
        ...
        FunctionBuilder(String signature) {
            ...
            this.inputTypes = params.isEmpty() ? new AbiType[0]
                    : parseParams(params);
        }
        ...
        public ContractFunction returns(String... types) {
            ...
            return new ContractFunction(address, client, Function.of(name, inputTypes).withReturns(out));
        }

        public ContractFunction returns(AbiType... types) {
            return new ContractFunction(address, client, Function.of(name, inputTypes).withReturns(types));
        }
