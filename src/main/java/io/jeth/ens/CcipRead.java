 ...
 *
 * When an ENS resolver reverts with {@code OffchainLookup(address,string[],bytes,bytes4,bytes)},
 * this class handles the gateway HTTP call and re-submits the result on-chain.
 *
 * The full flow:
 * <ol>
 *   <li>eth_call → resolver reverts with OffchainLookup</li>
 *   <li>CcipRead parses the revert data, picks a gateway URL</li>
 *   <li>HTTP GET/POST to the gateway with the calldata</li>
 *   <li>eth_call again with the callback selector + gateway response</li>
 * </ol>
 *
...
public final class CcipRead {
    ...
    /**
     * Execute an eth_call, and if it reverts with OffchainLookup, resolve via CCIP-Read.
     * Retries up to {@code maxDepth} times (to handle chained redirects).
     *
    ...
    public static CompletableFuture<String> call(
        ...
        EthModels.CallRequest req = EthModels.CallRequest.builder()
                .to(to).data(calldata).build();
        ...
        return client.send("eth_call", List.of(req, "latest"))
                .thenCompose(resp -> {
                    ...
                    if (revertData == null || !revertData.startsWith(OFFCHAIN_LOOKUP_SELECTOR)) {
                        // Not a CCIP-Read revert — propagate as normal error
                        throw new EthException(resp.errorMessage() != null
                                ? resp.errorMessage() : "eth_call failed", revertData);
                    }

                    if (maxDepth <= 0) {
                        throw new EthException("CCIP-Read: maximum redirect depth exceeded");
                    }
                    ...
                    return fetchGateway(lookup)
                            .thenCompose(gatewayResponse -> {
                                // Re-call with callbackFunction(bytes response, bytes extraData)
                                String callbackCalldata = buildCallbackCalldata(
                                        lookup.callbackFunction, gatewayResponse, lookup.extraData);
                                return call(client, to, callbackCalldata, maxDepth - 1);
                            });
                });
    }
    ...
    private static CompletableFuture<String> tryGateway(OffchainLookup lookup, int urlIndex) {
        ...
        String url = lookup.urls[urlIndex]
        ...
        if (isGet || !lookup.callData.isEmpty()) {
            ...
            if (url.contains("{data}") || url.contains("{sender}")) {
                ...
                String body = String.format(
                        "{\"data\":\"%s\",\"sender\":\"%s\"}", lookup.callData, lookup.sender);
        ...
        return request
                .thenApply(CcipRead::extractDataFromResponse)
    ...
    private static CompletableFuture<String> httpGet(String url) {
        ...
        return HTTP.sendAsync(req, HttpResponse.BodyHandlers.ofString())
                .thenApply(r -> {
                    if (r.statusCode() != 200)
                        throw new EthException("CCIP-Read gateway returned HTTP " + r.statusCode());
                    return r.body();
                });
    }
     ...
     *
     * ABI encoding of: OffchainLookup(address sender, string[] urls, bytes callData,
     *                                  bytes4 callbackFunction, bytes extraData)
     * after stripping the 4-byte selector.
     */
    static OffchainLookup parseOffchainLookup(String revertHex) {
        ...
        String callbackFunction = "0x" + Hex.encodeNoPrefx(
                Arrays.copyOfRange(data, 96, 100));
        ...
        for (int i = 0; i < urlCount; i++) {
            ...
            urls[i] = new String(Arrays.copyOfRange(
                    data, strOffset + 32, strOffset + 32 + strLen), StandardCharsets.UTF_8);
        }

        // Decode callData bytes
        int callDataLen = (int) decodeLong(data, callDataOffset);
        String callData = "0x" + Hex.encodeNoPrefx(
                Arrays.copyOfRange(data, callDataOffset + 32, callDataOffset + 32 + callDataLen));

        // Decode extraData bytes
        int extraDataLen = (int) decodeLong(data, extraDataOffset);
        String extraData = "0x" + Hex.encodeNoPrefx(
                Arrays.copyOfRange(data, extraDataOffset + 32, extraDataOffset + 32 + extraDataLen));
     ...
     */
    static String buildCallbackCalldata(String callbackFunction, String gatewayResponse, String extraData) {
