...
public final class Address {
     ...
     * Input can have or omit 0x prefix, any case.
     */
    public static String toChecksumAddress(String address) {
        if (address == null) throw new IllegalArgumentException("address is null");
        String addr = address.startsWith("0x") || address.startsWith("0X")
            ? address.substring(2) : address;
        if (addr.length() != 40) throw new IllegalArgumentException("Invalid address length: " + address);
    ...
    public static boolean isValid(String address) {
        ...
        String clean = address.startsWith("0x") || address.startsWith("0X")
            ? address.substring(2) : address;
    ...
    public static String validate(String address) {
        if (!isValid(address)) throw new IllegalArgumentException("Invalid Ethereum address: " + address);
        return address;
    }
    ...
    public static boolean isValidChecksum(String address) {
        if (!isValid(address)) return false;
        String clean = address.startsWith("0x") ? address.substring(2) : address;
        if (clean.equals(clean.toLowerCase()) || clean.equals(clean.toUpperCase(Locale.ROOT))) return false;
