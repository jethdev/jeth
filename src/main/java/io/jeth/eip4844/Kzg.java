 ...
 *
 * <ul>
 *   <li>{@link #blobToCommitment} — {@code blob_to_kzg_commitment}: 48-byte G1 commitment
 *   <li>{@link #computeBlobKzgProof} — {@code compute_blob_kzg_proof}: 48-byte KZG proof
 *   <li>{@link #computeChallenge} — {@code compute_challenge}: Fiat-Shamir hash
 *   <li>{@link #evaluatePolynomialInEvaluationForm} — barycentric evaluation
 * </ul>
 *
 * <p>The implementation matches the <a
 * href="https://github.com/ethereum/consensus-specs/blob/dev/specs/deneb/polynomial-commitments.md">
 * Ethereum consensus spec (Deneb)</a> exactly, including:
 *
 * <ul>
 *   <li>Using the <b>Lagrange (evaluation) form</b> of the trusted setup for both commitment and
 *       proof computation (not the monomial form)
 *   <li>The Fiat-Shamir challenge hash with correct {@code uint64} degree encoding
 *   <li>In-domain quotient formula for {@code z = ω^m} (uses sum-to-zero correction)
 * </ul>
 *
 * <h2>Usage</h2>
 *
 ...
 *
 * <ul>
 *   <li>Setup load: ~300ms (first call, then cached)
 *   <li>Commitment: ~8–15s (4096 G1 scalar multiplications)
 *   <li>Proof: ~8–15s
 * </ul>
 *
 * For high-throughput use, provide precomputed KZG from native c-kzg-4844 (~1ms/blob) and pass
 * directly to {@link BlobTransaction.Builder#blob(byte[], byte[], byte[])}.
 */
public final class Kzg {
     ...
     *
     * <p>Computed as {@code 7^((r-1)/2^32) mod r} where 7 is the generator of Fr*. Verified: {@code
     * ROOT_OF_UNITY_2_32^(2^32) ≡ 1 (mod r)} and {@code ROOT_OF_UNITY_2_32^(2^31) ≢ 1 (mod r)}
     * (primitive).
     */
    private static final BigInteger ROOT_OF_UNITY_2_32 =
            new BigInteger("16a2a19edfe81f20d09b681922c813b4b63683508c2280b93829971f439f0d2b", 16);
     ...
     *
     * <p>Spec: {@code blob_to_kzg_commitment(blob)}. Uses the Lagrange (evaluation) form of the
     * setup: {@code C = Σ_i poly[i] · G1_lagrange[i]}
     *
     ...
     *
     * <ol>
     *   <li>Compute Fiat-Shamir challenge {@code z = hash(blob ‖ commitment)}
     *   <li>Evaluate {@code y = p(z)} using the barycentric formula
     *   <li>Compute quotient polynomial {@code q} in evaluation form
     *   <li>Commit: {@code proof = Σ_i q[i] · G1_lagrange[i]}
     * </ol>
     *
    ...
    public static byte[] computeBlobKzgProof(byte[] blob, byte[] commitment) {
        ...
        if (commitment.length != 48)
            throw new IllegalArgumentException(
                    "commitment must be 48 bytes, got " + commitment.length);
     ...
     *
     * <p>Spec: {@code compute_challenge(polynomial, commitment)}. Hash: {@code
     * SHA256(FSBLOBVERIFY_V1_ ‖ degree_poly_as_uint64_be ‖ blob_bytes ‖ commitment)} Result: {@code
     * hash_to_bls_field(h) = int.from_bytes(h, 'big') mod r}
     */
    ...
    /**
     * Convert 131072 blob bytes to 4096 field elements. Each 32-byte chunk is a big-endian scalar
     * in Fr.
     */
     ...
     *
     * <p>Spec: {@code compute_quotient_eval_within_domain(z, poly, y)} (when z in domain) and
     * {@code compute_kzg_proof_impl} (general case).
     *
     * <p>Two cases:
     *
     * <ul>
     *   <li><b>z = ω^m</b> (in domain): {@code q[i] = (poly[i] - y) / (ω^i - ω^m)} for {@code i ≠
     *       m}, and {@code q[m]} uses the sum-to-zero correction.
     *   <li><b>z ∉ domain</b>: {@code q[i] = (poly[i] - y) / (ω^i - z)} for all i.
     * </ul>
     */
     ...
     *
     * <p>Uses {@link #ROOT_OF_UNITY_2_32} (a primitive 2^32-th root) and squares it {@code
     * log2(2^32/N)} times to get the primitive Nth root. N must be a power of 2 dividing 2^32.
     */
    static BigInteger rootOfUnity(int N) {
        // N must be a power of 2 dividing 2^32.
        // ROOT_OF_UNITY_2_32 has order 2^32; squaring it (32 - log2(N)) times gives a primitive Nth
        // root.
    ...
    private static void validateBlobSize(byte[] blob) {
        if (blob.length != BlobTransaction.BLOB_SIZE)
            throw new IllegalArgumentException(
                    "Blob must be exactly "
                            + BlobTransaction.BLOB_SIZE
                            + " bytes, got "
                            + blob.length);
    }
}
