 ...
 *
 * <p>Use {@link #from(byte[])} to create a blob from arbitrary data. This computes real KZG
 * commitment and proof using the Ethereum trusted setup — valid on Sepolia, Holesky, and Mainnet.
 *
 * <pre>
 ...
 * <h2>Trusted setup</h2>
 *
 * <p>The Ethereum KZG trusted setup is bundled in jeth's jar (from the ceremony at <a
 * href="https://ceremony.ethereum.org">ceremony.ethereum.org</a>). It is loaded lazily on the first
 * KZG call and cached for the process lifetime. You can supply a custom setup via the {@code
 * JETH_KZG_SETUP} environment variable.
 *
 * @see Kzg
 * @see KzgTrustedSetup
 * @see BlobTransaction
...
public final class Blob {
     ...
     * computes ~4096 G1 scalar multiplications (~8–15s pure Java). For performance-critical code,
     * use the native c-kzg-4844 library instead.
     *
     ...
     *
     * <p>Use this when you have commitment/proof from a native KZG library (e.g.
     * ethereum/c-kzg-4844 via JNI) for maximum performance.
     *
    ...
    public static Blob of(byte[] data, byte[] commitment, byte[] proof) {
        if (data.length != BYTES_PER_BLOB)
            throw new IllegalArgumentException("data must be " + BYTES_PER_BLOB + " bytes");
        if (commitment.length != BYTES_COMMITMENT)
            throw new IllegalArgumentException("commitment must be 48 bytes");
        if (proof.length != BYTES_PROOF)
            throw new IllegalArgumentException("proof must be 48 bytes");
    ...
    public String versionedHashHex() {
        return Hex.encode(versionedHash());
    }
