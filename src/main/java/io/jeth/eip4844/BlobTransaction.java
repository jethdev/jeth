 ...
 *
 * Blob transactions carry up to 6 blobs of ~128KB each (FIELD_ELEMENTS_PER_BLOB * 32 bytes).
 * Each blob has a KZG commitment and proof. On-chain only the commitment hash is stored;
 * the blob data is available on the beacon chain for ~18 days (4096 epochs).
 *
 * <pre>
...
public class BlobTransaction {
    ...
    private static final BigInteger BLS_MODULUS = new BigInteger(
    ...
    /**
     * Pad or truncate raw bytes to exactly BLOB_SIZE (131072 bytes).
     * Bytes are placed at the start; remainder is zero-padded.
     *
     * Note: each 32-byte chunk must be < BLS_MODULUS. For arbitrary data
     * use a real blob encoder library; this method is suitable for small payloads.
     */
     ...
     *
     * Note: In production you need a real KZG library (e.g. c-kzg-4844 via JNI or
     * ethereum/c-kzg-4844 Java bindings) to compute valid commitments and proofs.
     * This implementation uses placeholder commitments for testing with devnets.
     */
    public String sign(Wallet wallet) {
        ...
        List<Object> txFields = new ArrayList<>(List.of(
        ...
        // Correct structure: [[chainId, nonce, ..., v, r, s], [blob1,...], [comm1,...], [proof1,...]]
    ...
    public List<byte[]> getBlobVersionedHashes() { return blobVersionedHashes; }
    public List<byte[]> getBlobs()               { return blobs; }
    public List<byte[]> getCommitments()         { return commitments; }
    public List<byte[]> getProofs()              { return proofs; }
    ...
    public static class Builder {
        ...
        public Builder gas(BigInteger v)                 { this.gas = v; return this; }
        public Builder maxFeePerGas(BigInteger v)        { this.maxFeePerGas = v; return this; }
        public Builder maxPriorityFeePerGas(BigInteger v){ this.maxPriorityFeePerGas = v; return this; }
        public Builder maxFeePerBlobGas(BigInteger v)    { this.maxFeePerBlobGas = v; return this; }

        /**
         * Add a blob with its KZG commitment and proof.
         * blob must be exactly BLOB_SIZE (131072) bytes.
         * commitment and proof must be exactly 48 bytes each.
         */
        public Builder blob(byte[] blob, byte[] commitment, byte[] proof) {
            if (blob.length != BLOB_SIZE)
                throw new EthException("Blob must be exactly " + BLOB_SIZE + " bytes, got " + blob.length);
            if (commitment.length != 48)
                throw new EthException("KZG commitment must be 48 bytes");
            if (proof.length != 48)
                throw new EthException("KZG proof must be 48 bytes");
         ...
         *
         * <p>This is the recommended API — commitment and proof are already
         * computed inside the {@link Blob} object, valid on Sepolia and Mainnet.
         *
         * <pre>
         ...
         *
         * <p><b>NOT valid on Sepolia, Holesky, or Mainnet.</b> Use {@link #blob(Blob)}
         * which calls {@link Blob#from(byte[])} to compute real KZG automatically.
         *
        ...
        public String sign(Wallet wallet) {
            if (to == null) throw new EthException("Blob transaction must have a 'to' address");
            if (blobs.isEmpty()) throw new EthException("Blob transaction must have at least one blob");
