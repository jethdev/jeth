...
public class UniswapV3 {
    ...
    public static final String SWAP_ROUTER_02_ADDRESS = "0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45";
    ...
    public UniswapV3(EthClient client) {
        ...
        this.fnQuoteExactInputSingle = quoter
                .fn("quoteExactInputSingle((address,address,uint256,uint24,uint160))")
    ...
    public CompletableFuture<PoolState> getPoolState(String tokenA, String tokenB, int fee) {
        return getPoolAddress(tokenA, tokenB, fee).thenCompose(poolAddr -> {
            if (poolAddr == null || Address.isZero(poolAddr))
                return CompletableFuture.failedFuture(
                        new EthException("Pool does not exist for " + tokenA + "/" + tokenB + "/" + fee));
            var pool = new Contract(poolAddr, client);
            var slot0 = pool.fn("slot0()").returns("uint160", "int24", "uint16", "uint16", "uint16", "uint8", "bool");
            var liq   = pool.fn("liquidity()").returns("uint128");
            return slot0.call().raw().thenCompose(s0 ->
                liq.call().as(BigInteger.class).thenApply(liquidity ->
                    new PoolState(poolAddr, (BigInteger) s0[0], ((BigInteger) s0[1]).intValue(), liquidity)));
        });
    }

    /** Execute exact-input single swap. */
    public CompletableFuture<String> swapExactInputSingle(
            Wallet wallet, String tokenIn, String tokenOut, int fee,
            BigInteger amountIn, BigInteger amountOutMinimum) {
        Object[] params = {tokenIn, tokenOut, BigInteger.valueOf(fee),
                wallet.getAddress(), amountIn, amountOutMinimum, BigInteger.ZERO};
        return router.fn("exactInputSingle((address,address,uint24,address,uint256,uint256,uint160))")
                     .send(wallet, params);
    }
    ...
    public CompletableFuture<List<BigInteger>> batchQuote(List<QuoteParams> quotes) {
        ...
        Function qFn = Function.of("quoteExactInputSingle",
                AbiType.of("(address,address,uint256,uint24,uint160)"))
                .withReturns(AbiType.UINT256);
        for (QuoteParams q : quotes) {
            Object[] tuple = {q.tokenIn(), q.tokenOut(), q.amountIn(), BigInteger.valueOf(q.fee()), BigInteger.ZERO};
            mc.addAllowFailure(QUOTER_V2_ADDRESS, qFn, new Object[]{tuple});
        }
        return mc.execute().thenApply(results ->
                results.stream().map(r -> r instanceof BigInteger bi ? bi : BigInteger.ZERO).toList());
    }

    public record PoolState(String address, BigInteger sqrtPriceX96, int tick, BigInteger liquidity) {
